...
s.t.  eqSupTotal{(s1, c, r) in mvSupReserve}: vSupReserve[s1,c,r]  =  sum{y in year,s in slice:((s1,c,r,y,s) in mSupAva)}((pPeriodLen[y]*vSupOut[s1,c,r,y,s]) / (pYearFraction[y]));
