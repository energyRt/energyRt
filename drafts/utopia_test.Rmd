...
# set_gams_path("/Applications/GAMS25.1/sysdir")
# get_gams_path()
# set_gdxlib_path("/Library/Frameworks/GAMS.framework/Versions/45/Resources/")
# get_gdxlib_path()
...
scen_BASE <- solve(mdl, "BASE",
  ...
  solver = solver_options$gams_gdx_cplex,
    # solver = solver_options$gams_csv_cplex,
...
# Rglpk::Rglpk_read_file("solwork/UTOPIA_BASE_GLPK_debug/energyRt.mod")
...
scen_COSTSGE0 <- solve(
  add(mdl, COSTSGE0), 
  name = "CO2CAP",
  tmp.dir = "solwork/UTOPIA_COSTSGE0",
  # solver = mysolver,
  # solver = "GLPK",
  # solver = GAMS_csv,
  # solver = GAMS_simplex,
...
CO2CAP <- newConstraint(
  ...
  rhs = data.frame(
    ...
    rhs = approx(
      x = c(2030, 2060),
      y = round(emis_co2$value, -3) * c(1, .1), 
      xout = c(2030:2060)
    )$y
  )
)
...
CO2CAP <- newConstraint(
  ...
  rhs = data.frame(
    # region = CO2_cap$V1,
    year = c(2030, 2060),
    rhs = round(emis_co2$value, -3) * c(1, .1)
  )
)
