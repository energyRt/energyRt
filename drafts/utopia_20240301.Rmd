---
title: 'Utopia: the testing model (transitional/interim version)'
...
# Intoduction
*[The text and parts of code might be obsolete - the update is in progress]*  
...
```{r setup}
if (packageVersion("energyRt") < "0.11.21") {
...
library(lubridate)
library(data.table)
try(source("~/.energyRt/settings.R"), silent = T) # (optional) load personal settings
...
# alternative way to define the constraint, using newConstraintS
CO2CAP2 <- newConstraintS(
  "CO2CAP2", 
  type = "balance",
  eq = "<=",
  for.each = list(year = c(2030:2060), comm = "CO2"),
  rhs = data.frame(
    year = c(2030:2060),
    rhs = 1e4
  )
