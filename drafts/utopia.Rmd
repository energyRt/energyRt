...
if (T) {
  ...
  # energyRt
  if (!any(mypks == "energyRt")) devtools::install_github("energyRt/energyRt")
  # GIS tools
...
# set_gdxlib_path("C:/GAMS/win64/32.2")
...
GAMS <- list(
  ...
  cmdline = "C:/GAMS/win64/32.2/gams.exe energyRt.gms",
  # solver = "cbc",
  solver = "cplex"
)
...
SO2 <- newCommodity("SO2", desc = "Sulfur dioxide emissions", slice = "HOUR")
HG <- newCommodity("HG", desc = "Mercury emissions", slice = "HOUR")
PM <- newCommodity("PM", desc = "Particulate matter emissions", slice = "HOUR")
```
...
scen_BASE_py <- write_sc(scen_BASE_int,
  # solver = "PYOMO",
  solver = Pyomo,
  tmp.dir = "tmp/solwork/UTOPIA_BASE_PYOMO_debug_sql"
)
...
vTechCap <- getData(scen, "vTechCap", merge = T) 

vTechOutTot <- getData(scen, "vTechOutTot", comm = "ELC", merge = T, 
                       process = T) |>
  group_by(scenario, name,region, year) |>
  summarize(value = sum(value, na.rm = T), .groups = "drop") |>
  pivot_wider()

vTechOut <- getData(scen, "vTechOut", comm = "ELC", merge = T, process = T) |>
  group_by(scenario, name, process, region, year) |>
  summarize(value = sum(value, na.rm = T), .groups = "drop") |>
  pivot_wider()

vDemInp <- getData(scen, "vDemInp", comm = "ELC", merge = T) |>
  group_by(scenario, name, region, year) |>
  summarize(value = sum(value, na.rm = T), .groups = "drop") |>
  pivot_wider()

mktShareDem <- vTechOut |> left_join(vDemInp) |>
  mutate(share = vTechOut / vDemInp)
summary(mktShareDem$share)

mktShareOut <- vTechOut |> left_join(vTechOutTot) |>
  mutate(share = vTechOut / vTechOutTot)
summary(mktShareOut$share)

ggplot(mktShareOut) +
  # geom_area(aes(year, share), stat = "identity") +
  geom_line(aes(year, share), stat = "identity") +
...
CSHARE_EGAS <- newConstraint(
      name = "CSHARE_EGAS",
      eq = "<=",
      for.each = data.frame(
        year = cn_dim$year,
        region = cn_dim$region,
        tech = "EGAS",
        comm = "ELC"
      ),
      tr1 = list(
...
scen_CSHARE <- interpolate_model(
  mdl, CSHARE_EGAS,
  name = "CSHARE_EGAS"
)

scen_CSHARE <- write_sc(
  # scen,
  scen_CSHARE,
  solver = GAMS,
  # solver = GAMS_csv,
  # solver = GAMS_simplex,
  # solver = GAMS_barrier,
  tmp.dir = "tmp/solwork/UTOPIA_CSHARE"
)
scen_CSHARE@misc$tmp.dir # directory

scen_CSHARE <- solve(scen_CSHARE, wait = F)
scen_CSHARE <- read(scen_CSHARE)
