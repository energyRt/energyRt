...
model.eqOutTot = Constraint(model.mvOutTot, rule = lambda model, c, r, y, s : model.vOutTot[c,r,y,s]  ==  (model.vDummyImport[c,r,y,s] if (c,r,y,s) in model.mDummyImport else 0)+(model.vSupOutTot[c,r,y,s] if (c,r,y,s) in model.mSupOutTot else 0)+(model.vEmsFuelTot[c,r,y,s] if (c,r,y,s) in model.mEmsFuelTot else 0)+(model.vAggOut[c,r,y,s] if (c,r,y,s) in model.mAggOut else 0)+(model.vTechOutTot[c,r,y,s] if (c,r,y,s) in model.mTechOutTot else 0)+(model.vStorageOutTot[c,r,y,s] if (c,r,y,s) in model.mStorageOutTot else 0)+(model.vImport[c,r,y,s] if (c,r,y,s) in model.mImport else 0)+(model.vTradeIrAOutTot[c,r,y,s] if (c,r,y,s) in model.mvTradeIrAOutTot else 0)+(sum(model.vOut2Lo[c,r,y,sp,s] for sp in model.slice if ((sp,s) in model.mSliceParentChild and (c,r,y,sp,s) in model.mvOut2Lo)) if (c,r,y,s) in model.mOutSub else 0));
