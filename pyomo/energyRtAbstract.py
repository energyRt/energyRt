...
model.eqBal = Constraint(model.mvBalance, rule = lambda model, c, r, y, s : model.vBalance[c,r,y,s]  ==  (model.vOutTot[c,r,y,s] if (c,r,y,s) in model.mvOutTot else 0)-(model.vInpTot[c,r,y,s] if (c,r,y,s) in model.mvInpTot else 0));
...
model.eqSubsCost = Constraint(model.mSubCost, rule = lambda model, c, r, y : model.vSubsCost[c,r,y]  ==  sum(model.pSubCostOut[c,r,y,s]*model.vOutTot[c,r,y,s] for s in model.slice if ((c,r,y,s) in model.mvOutTot and (c,s) in model.mCommSlice))+sum(model.pSubCostInp[c,r,y,s]*model.vInpTot[c,r,y,s] for s in model.slice if ((c,r,y,s) in model.mvInpTot and (c,s) in model.mCommSlice))+sum(model.pSubCostBal[c,r,y,s]*model.vBalance[c,r,y,s] for s in model.slice if ((c,r,y,s) in model.mvBalance and (c,s) in model.mCommSlice)));
# eqObjective
