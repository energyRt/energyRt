...
model.eqTaxCost = Constraint(model.mTaxCost, rule = lambda model, c, r, y : model.vTaxCost[c,r,y]  ==  sum(model.pTaxCost[c,r,y,s]*model.vOutTot[c,r,y,s] for s in model.slice if ((c,r,y,s) in model.mvOutTot and (c,s) in model.mCommSlice)));
# eqSubsCost(comm, region, year)$mSubsCost(comm, region, year)
model.eqSubsCost = Constraint(model.mSubsCost, rule = lambda model, c, r, y : model.vSubsCost[c,r,y]  ==  sum(model.pSubsCost[c,r,y,s]*model.vOutTot[c,r,y,s] for s in model.slice if ((c,s) in model.mCommSlice and (c,r,y,s) in model.mvOutTot)));
# eqObjective
