...
model.vAggOutTot = Var(model.mAggOut, doc = "Aggregated commodity output");
...
model.vImportTot = Var(model.mImport, domain = pyo.NonNegativeReals, doc = "Total regional import (Ir + ROW)");
model.vExportTot = Var(model.mExport, domain = pyo.NonNegativeReals, doc = "Total regional export (Ir + ROW)");
...
model.eqTechAInp = Constraint(model.mvTechAInp, rule = lambda model, t, c, r, y, s : model.vTechAInp[t,c,r,y,s]  ==  ((model.vTechAct[t,r,y,s]*model.pTechAct2AInp[t,c,r,y,s]) if (t,c,r,y,s) in model.mTechAct2AInp else 0)+((model.vTechCap[t,r,y]*model.pTechCap2AInp[t,c,r,y,s]) if (t,c,r,y,s) in model.mTechCap2AInp else 0)+((model.vTechNewCap[t,r,y]*model.pTechNCap2AInp[t,c,r,y,s]) if (t,c,r,y,s) in model.mTechNCap2AInp else 0)+sum(model.pTechCinp2AInp[t,c,cp,r,y,s]*model.vTechInp[t,cp,r,y,s] for cp in model.comm if (t,c,cp,r,y,s) in model.mTechCinp2AInp)+sum(model.pTechCout2AInp[t,c,cp,r,y,s]*model.vTechOut[t,cp,r,y,s] for cp in model.comm if (t,c,cp,r,y,s) in model.mTechCout2AInp));
# eqTechAOut(tech, comm, region, year, slice)$mvTechAOut(tech, comm, region, year, slice)
model.eqTechAOut = Constraint(model.mvTechAOut, rule = lambda model, t, c, r, y, s : model.vTechAOut[t,c,r,y,s]  ==  ((model.vTechAct[t,r,y,s]*model.pTechAct2AOut[t,c,r,y,s]) if (t,c,r,y,s) in model.mTechAct2AOut else 0)+((model.vTechCap[t,r,y]*model.pTechCap2AOut[t,c,r,y,s]) if (t,c,r,y,s) in model.mTechCap2AOut else 0)+((model.vTechNewCap[t,r,y]*model.pTechNCap2AOut[t,c,r,y,s]) if (t,c,r,y,s) in model.mTechNCap2AOut else 0)+sum(model.pTechCinp2AOut[t,c,cp,r,y,s]*model.vTechInp[t,cp,r,y,s] for cp in model.comm if (t,c,cp,r,y,s) in model.mTechCinp2AOut)+sum(model.pTechCout2AOut[t,c,cp,r,y,s]*model.vTechOut[t,cp,r,y,s] for cp in model.comm if (t,c,cp,r,y,s) in model.mTechCout2AOut));
# eqTechAfLo(tech, region, year, slice)$meqTechAfLo(tech, region, year, slice)
model.eqTechAfLo = Constraint(model.meqTechAfLo, rule = lambda model, t, r, y, s : model.pTechAfLo[t,r,y,s]*model.pTechCap2act[t]*model.vTechCap[t,r,y]*model.pSliceShare[s]*prod(model.pTechWeatherAfLo[wth1,t]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,t) in model.mTechWeatherAfLo) <=  model.vTechAct[t,r,y,s]);
# eqTechAfUp(tech, region, year, slice)$meqTechAfUp(tech, region, year, slice)
model.eqTechAfUp = Constraint(model.meqTechAfUp, rule = lambda model, t, r, y, s : model.vTechAct[t,r,y,s] <=  model.pTechAfUp[t,r,y,s]*model.pTechCap2act[t]*model.vTechCap[t,r,y]*model.pSliceShare[s]*prod(model.pTechWeatherAfUp[wth1,t]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,t) in model.mTechWeatherAfUp));
# eqTechAfsLo(tech, region, year, slice)$meqTechAfsLo(tech, region, year, slice)
model.eqTechAfsLo = Constraint(model.meqTechAfsLo, rule = lambda model, t, r, y, s : model.pTechAfsLo[t,r,y,s]*model.pTechCap2act[t]*model.vTechCap[t,r,y]*model.pSliceShare[s]*prod(model.pTechWeatherAfsLo[wth1,t]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,t) in model.mTechWeatherAfsLo) <=  sum((model.vTechAct[t,r,y,sp] if (t,r,y,sp) in model.mvTechAct else 0) for sp in model.slice if (s,sp) in model.mSliceParentChildE));
# eqTechAfsUp(tech, region, year, slice)$meqTechAfsUp(tech, region, year, slice)
model.eqTechAfsUp = Constraint(model.meqTechAfsUp, rule = lambda model, t, r, y, s : sum((model.vTechAct[t,r,y,sp] if (t,r,y,sp) in model.mvTechAct else 0) for sp in model.slice if (s,sp) in model.mSliceParentChildE) <=  model.pTechAfsUp[t,r,y,s]*model.pTechCap2act[t]*model.vTechCap[t,r,y]*model.pSliceShare[s]*prod(model.pTechWeatherAfsUp[wth1,t]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,t) in model.mTechWeatherAfsUp));
# eqTechRampUp(tech, region, year, slice, slicep)$mTechRampUp(tech, region, year, slice, slicep)
model.eqTechRampUp = Constraint(model.mTechRampUp, rule = lambda model, t, r, y, s, sp : (model.vTechAct[t,r,y,s]) / (model.pSliceShare[s])-(model.vTechAct[t,r,y,sp]) / (model.pSliceShare[sp]) <=  (model.pSliceShare[s]*model.pTechCap2act[t]*model.pTechCap2act[t]*model.vTechCap[t,r,y]) / (model.pTechRampUp[t,r,y,s]));
# eqTechRampDown(tech, region, year, slice, slicep)$mTechRampDown(tech, region, year, slice, slicep)
model.eqTechRampDown = Constraint(model.mTechRampDown, rule = lambda model, t, r, y, s, sp : (model.vTechAct[t,r,y,sp]) / (model.pSliceShare[sp])-(model.vTechAct[t,r,y,s]) / (model.pSliceShare[s]) <=  (model.pSliceShare[s]*model.pTechCap2act[t]*model.pTechCap2act[t]*model.vTechCap[t,r,y]) / (model.pTechRampDown[t,r,y,s]));
...
model.eqTechAfcOutLo = Constraint(model.meqTechAfcOutLo, rule = lambda model, t, r, c, y, s : model.pTechCact2cout[t,c,r,y,s]*model.pTechAfcLo[t,c,r,y,s]*model.pTechCap2act[t]*model.vTechCap[t,r,y]*model.pSliceShare[s]*prod(model.pTechWeatherAfcLo[wth1,t,c]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,t,c) in model.mTechWeatherAfcLo) <=  model.vTechOut[t,c,r,y,s]);
# eqTechAfcOutUp(tech, region, comm, year, slice)$meqTechAfcOutUp(tech, region, comm, year, slice)
model.eqTechAfcOutUp = Constraint(model.meqTechAfcOutUp, rule = lambda model, t, r, c, y, s : model.vTechOut[t,c,r,y,s] <=  model.pTechCact2cout[t,c,r,y,s]*model.pTechAfcUp[t,c,r,y,s]*model.pTechCap2act[t]*model.vTechCap[t,r,y]*prod(model.pTechWeatherAfcUp[wth1,t,c]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,t,c) in model.mTechWeatherAfcUp));
# eqTechAfcInpLo(tech, region, comm, year, slice)$meqTechAfcInpLo(tech, region, comm, year, slice)
model.eqTechAfcInpLo = Constraint(model.meqTechAfcInpLo, rule = lambda model, t, r, c, y, s : model.pTechAfcLo[t,c,r,y,s]*model.pTechCap2act[t]*model.vTechCap[t,r,y]*model.pSliceShare[s]*prod(model.pTechWeatherAfcLo[wth1,t,c]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,t,c) in model.mTechWeatherAfcLo) <=  model.vTechInp[t,c,r,y,s]);
# eqTechAfcInpUp(tech, region, comm, year, slice)$meqTechAfcInpUp(tech, region, comm, year, slice)
model.eqTechAfcInpUp = Constraint(model.meqTechAfcInpUp, rule = lambda model, t, r, c, y, s : model.vTechInp[t,c,r,y,s] <=  model.pTechAfcUp[t,c,r,y,s]*model.pTechCap2act[t]*model.vTechCap[t,r,y]*model.pSliceShare[s]*prod(model.pTechWeatherAfcUp[wth1,t,c]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,t,c) in model.mTechWeatherAfcUp));
# eqTechCap(tech, region, year)$mTechSpan(tech, region, year)
model.eqTechCap = Constraint(model.mTechSpan, rule = lambda model, t, r, y : model.vTechCap[t,r,y]  ==  model.pTechStock[t,r,y]-(model.vTechRetiredStock[t,r,y] if (t,r,y) in model.mvTechRetiredStock else 0)+sum(model.pPeriodLen[yp]*(model.vTechNewCap[t,r,yp]-sum(model.vTechRetiredNewCap[t,r,yp,ye] for ye in model.year if ((t,r,yp,ye) in model.mvTechRetiredNewCap and model.ordYear[y] >= model.ordYear[ye]))) for yp in model.year if ((t,r,yp) in model.mTechNew and model.ordYear[y] >= model.ordYear[yp] and (model.ordYear[y]<model.pTechOlife[t,r]+model.ordYear[yp] or (t,r) in model.mTechOlifeInf))));
# eqTechRetiredNewCap(tech, region, year)$meqTechRetiredNewCap(tech, region, year)
model.eqTechRetiredNewCap = Constraint(model.meqTechRetiredNewCap, rule = lambda model, t, r, y : sum(model.vTechRetiredNewCap[t,r,y,yp] for yp in model.year if (t,r,y,yp) in model.mvTechRetiredNewCap) <=  model.vTechNewCap[t,r,y]);
...
model.eqTechEac = Constraint(model.mTechEac, rule = lambda model, t, r, y : model.vTechEac[t,r,y]  ==  sum(model.pTechEac[t,r,yp]*model.pPeriodLen[yp]*(model.vTechNewCap[t,r,yp]-sum(model.vTechRetiredNewCap[t,r,yp,ye] for ye in model.year if ((t,r,yp,ye) in model.mvTechRetiredNewCap and model.ordYear[y] >= model.ordYear[ye]))) for yp in model.year if ((t,r,yp) in model.mTechNew and model.ordYear[y] >= model.ordYear[yp] and (model.ordYear[y]<model.pTechOlife[t,r]+model.ordYear[yp] or (t,r) in model.mTechOlifeInf))));
...
model.eqTechOMCost = Constraint(model.mTechOMCost, rule = lambda model, t, r, y : model.vTechOMCost[t,r,y]  ==  model.pTechFixom[t,r,y]*model.vTechCap[t,r,y]+sum(model.pTechVarom[t,r,y,s]*model.pSliceWeight[s]*model.vTechAct[t,r,y,s]+sum(model.pTechCvarom[t,c,r,y,s]*model.pSliceWeight[s]*model.vTechInp[t,c,r,y,s] for c in model.comm if (t,c) in model.mTechInpComm)+sum(model.pTechCvarom[t,c,r,y,s]*model.pSliceWeight[s]*model.vTechOut[t,c,r,y,s] for c in model.comm if (t,c) in model.mTechOutComm)+sum(model.pTechAvarom[t,c,r,y,s]*model.pSliceWeight[s]*model.vTechAOut[t,c,r,y,s] for c in model.comm if (t,c,r,y,s) in model.mvTechAOut)+sum(model.pTechAvarom[t,c,r,y,s]*model.pSliceWeight[s]*model.vTechAInp[t,c,r,y,s] for c in model.comm if (t,c,r,y,s) in model.mvTechAInp) for s in model.slice if (t,s) in model.mTechSlice));
# eqSupAvaUp(sup, comm, region, year, slice)$mSupAvaUp(sup, comm, region, year, slice)
model.eqSupAvaUp = Constraint(model.mSupAvaUp, rule = lambda model, s1, c, r, y, s : model.vSupOut[s1,c,r,y,s] <=  model.pSupAvaUp[s1,c,r,y,s]*prod(model.pSupWeatherUp[wth1,s1]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,s1) in model.mSupWeatherUp));
# eqSupAvaLo(sup, comm, region, year, slice)$meqSupAvaLo(sup, comm, region, year, slice)
model.eqSupAvaLo = Constraint(model.meqSupAvaLo, rule = lambda model, s1, c, r, y, s : model.vSupOut[s1,c,r,y,s]  >=  model.pSupAvaLo[s1,c,r,y,s]*prod(model.pSupWeatherLo[wth1,s1]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,s1) in model.mSupWeatherLo));
# eqSupTotal(sup, comm, region)$mvSupReserve(sup, comm, region)
model.eqSupTotal = Constraint(model.mvSupReserve, rule = lambda model, s1, c, r : model.vSupReserve[s1,c,r]  ==  sum(model.pPeriodLen[y]*model.pSliceWeight[s]*model.vSupOut[s1,c,r,y,s] for y in model.year for s in model.slice if (s1,c,r,y,s) in model.mSupAva));
# eqSupReserveUp(sup, comm, region)$mSupReserveUp(sup, comm, region)
model.eqSupReserveUp = Constraint(model.mSupReserveUp, rule = lambda model, s1, c, r : model.vSupReserve[s1,c,r] <=  model.pSupReserveUp[s1,c,r]);
...
model.eqSupCost = Constraint(model.mvSupCost, rule = lambda model, s1, r, y : model.vSupCost[s1,r,y]  ==  sum(model.pSupCost[s1,c,r,y,s]*model.pSliceWeight[s]*model.vSupOut[s1,c,r,y,s] for c in model.comm for s in model.slice if (s1,c,r,y,s) in model.mSupAva));
...
model.eqAggOut = Constraint(model.mAggOut, rule = lambda model, c, r, y, s : model.vAggOutTot[c,r,y,s]  ==  sum(model.pAggregateFactor[c,cp]*sum(model.vOutTot[cp,r,y,sp] for sp in model.slice if ((c,r,y,sp) in model.mvOutTot and (s,sp) in model.mSliceParentChildE and (cp,sp) in model.mCommSlice)) for cp in model.comm if (c,cp) in model.mAggregateFactor));
# eqEmsFuelTot(comm, region, year, slice)$mEmsFuelTot(comm, region, year, slice)
model.eqEmsFuelTot = Constraint(model.mEmsFuelTot, rule = lambda model, c, r, y, s : model.vEmsFuelTot[c,r,y,s]  ==  sum(model.pEmissionFactor[c,cp]*sum(model.pTechEmisComm[t,cp]*sum((model.vTechInp[t,cp,r,y,sp] if (t,c,cp,r,y,sp) in model.mTechEmsFuel else 0) for sp in model.slice if (c,s,sp) in model.mCommSliceOrParent) for t in model.tech if (t,cp) in model.mTechInpComm) for cp in model.comm if (model.pEmissionFactor[c,cp]>0))*model.pSliceWeight[s]);
# eqStorageAInp(stg, comm, region, year, slice)$mvStorageAInp(stg, comm, region, year, slice)
model.eqStorageAInp = Constraint(model.mvStorageAInp, rule = lambda model, st1, c, r, y, s : model.vStorageAInp[st1,c,r,y,s]  ==  sum(((model.pStorageStg2AInp[st1,c,r,y,s]*model.vStorageStore[st1,cp,r,y,s]) if (st1,c,r,y,s) in model.mStorageStg2AInp else 0)+((model.pStorageCinp2AInp[st1,c,r,y,s]*model.vStorageInp[st1,cp,r,y,s]) if (st1,c,r,y,s) in model.mStorageCinp2AInp else 0)+((model.pStorageCout2AInp[st1,c,r,y,s]*model.vStorageOut[st1,cp,r,y,s]) if (st1,c,r,y,s) in model.mStorageCout2AInp else 0)+((model.pStorageCap2AInp[st1,c,r,y,s]*model.vStorageCap[st1,r,y]) if (st1,c,r,y,s) in model.mStorageCap2AInp else 0)+((model.pStorageNCap2AInp[st1,c,r,y,s]*model.vStorageNewCap[st1,r,y]) if (st1,c,r,y,s) in model.mStorageNCap2AInp else 0) for cp in model.comm if (st1,cp) in model.mStorageComm));
# eqStorageAOut(stg, comm, region, year, slice)$mvStorageAOut(stg, comm, region, year, slice)
model.eqStorageAOut = Constraint(model.mvStorageAOut, rule = lambda model, st1, c, r, y, s : model.vStorageAOut[st1,c,r,y,s]  ==  sum(((model.pStorageStg2AOut[st1,c,r,y,s]*model.vStorageStore[st1,cp,r,y,s]) if (st1,c,r,y,s) in model.mStorageStg2AOut else 0)+((model.pStorageCinp2AOut[st1,c,r,y,s]*model.vStorageInp[st1,cp,r,y,s]) if (st1,c,r,y,s) in model.mStorageCinp2AOut else 0)+((model.pStorageCout2AOut[st1,c,r,y,s]*model.vStorageOut[st1,cp,r,y,s]) if (st1,c,r,y,s) in model.mStorageCout2AOut else 0)+((model.pStorageCap2AOut[st1,c,r,y,s]*model.vStorageCap[st1,r,y]) if (st1,c,r,y,s) in model.mStorageCap2AOut else 0)+((model.pStorageNCap2AOut[st1,c,r,y,s]*model.vStorageNewCap[st1,r,y]) if (st1,c,r,y,s) in model.mStorageNCap2AOut else 0) for cp in model.comm if (st1,cp) in model.mStorageComm));
...
model.eqStorageAfLo = Constraint(model.meqStorageAfLo, rule = lambda model, st1, c, r, y, s : model.vStorageStore[st1,c,r,y,s]  >=  model.pStorageAfLo[st1,r,y,s]*model.pStorageCap2stg[st1]*model.vStorageCap[st1,r,y]*prod(model.pStorageWeatherAfLo[wth1,st1]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,st1) in model.mStorageWeatherAfLo));
# eqStorageAfUp(stg, comm, region, year, slice)$meqStorageAfUp(stg, comm, region, year, slice)
model.eqStorageAfUp = Constraint(model.meqStorageAfUp, rule = lambda model, st1, c, r, y, s : model.vStorageStore[st1,c,r,y,s] <=  model.pStorageAfUp[st1,r,y,s]*model.pStorageCap2stg[st1]*model.vStorageCap[st1,r,y]*prod(model.pStorageWeatherAfUp[wth1,st1]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,st1) in model.mStorageWeatherAfUp));
...
model.eqStorageInpUp = Constraint(model.meqStorageInpUp, rule = lambda model, st1, c, r, y, s : model.vStorageInp[st1,c,r,y,s] <=  model.pStorageCap2stg[st1]*model.vStorageCap[st1,r,y]*model.pStorageCinpUp[st1,c,r,y,s]*model.pSliceShare[s]*prod(model.pStorageWeatherCinpUp[wth1,st1]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,st1) in model.mStorageWeatherCinpUp));
# eqStorageInpLo(stg, comm, region, year, slice)$meqStorageInpLo(stg, comm, region, year, slice)
model.eqStorageInpLo = Constraint(model.meqStorageInpLo, rule = lambda model, st1, c, r, y, s : model.vStorageInp[st1,c,r,y,s]  >=  model.pStorageCap2stg[st1]*model.vStorageCap[st1,r,y]*model.pStorageCinpLo[st1,c,r,y,s]*model.pSliceShare[s]*prod(model.pStorageWeatherCinpLo[wth1,st1]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,st1) in model.mStorageWeatherCinpLo));
# eqStorageOutUp(stg, comm, region, year, slice)$meqStorageOutUp(stg, comm, region, year, slice)
model.eqStorageOutUp = Constraint(model.meqStorageOutUp, rule = lambda model, st1, c, r, y, s : model.vStorageOut[st1,c,r,y,s] <=  model.pStorageCap2stg[st1]*model.vStorageCap[st1,r,y]*model.pStorageCoutUp[st1,c,r,y,s]*model.pSliceShare[s]*prod(model.pStorageWeatherCoutUp[wth1,st1]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,st1) in model.mStorageWeatherCoutUp));
# eqStorageOutLo(stg, comm, region, year, slice)$meqStorageOutLo(stg, comm, region, year, slice)
model.eqStorageOutLo = Constraint(model.meqStorageOutLo, rule = lambda model, st1, c, r, y, s : model.vStorageOut[st1,c,r,y,s]  >=  model.pStorageCap2stg[st1]*model.vStorageCap[st1,r,y]*model.pStorageCoutLo[st1,c,r,y,s]*model.pSliceShare[s]*prod(model.pStorageWeatherCoutLo[wth1,st1]*model.pWeather[wth1,r,y,s] for wth1 in model.weather if (wth1,st1) in model.mStorageWeatherCoutLo));
# eqStorageCap(stg, region, year)$mStorageSpan(stg, region, year)
model.eqStorageCap = Constraint(model.mStorageSpan, rule = lambda model, st1, r, y : model.vStorageCap[st1,r,y]  ==  model.pStorageStock[st1,r,y]+sum(model.pPeriodLen[yp]*model.vStorageNewCap[st1,r,yp] for yp in model.year if (model.ordYear[y] >= model.ordYear[yp] and ((st1,r) in model.mStorageOlifeInf or model.ordYear[y]<model.pStorageOlife[st1,r]+model.ordYear[yp]) and (st1,r,yp) in model.mStorageNew)));
...
model.eqStorageEac = Constraint(model.mStorageEac, rule = lambda model, st1, r, y : model.vStorageEac[st1,r,y]  ==  sum(model.pStorageEac[st1,r,yp]*model.pPeriodLen[yp]*model.vStorageNewCap[st1,r,yp] for yp in model.year if ((st1,r,yp) in model.mStorageNew and model.ordYear[y] >= model.ordYear[yp] and ((st1,r) in model.mStorageOlifeInf or model.ordYear[y]<model.pStorageOlife[st1,r]+model.ordYear[yp]) and model.pStorageInvcost[st1,r,yp] != 0)));
# eqStorageCost(stg, region, year)$mStorageOMCost(stg, region, year)
model.eqStorageCost = Constraint(model.mStorageOMCost, rule = lambda model, st1, r, y : model.vStorageOMCost[st1,r,y]  ==  model.pStorageFixom[st1,r,y]*model.vStorageCap[st1,r,y]+sum(sum(model.pStorageCostInp[st1,r,y,s]*model.pSliceWeight[s]*model.vStorageInp[st1,c,r,y,s]+model.pStorageCostOut[st1,r,y,s]*model.pSliceWeight[s]*model.vStorageOut[st1,c,r,y,s]+model.pStorageCostStore[st1,r,y,s]*model.pSliceWeight[s]*model.vStorageStore[st1,c,r,y,s] for s in model.slice if (c,s) in model.mCommSlice) for c in model.comm if (st1,c) in model.mStorageComm));
# eqImport(comm, dst, year, slice)$mImport(comm, dst, year, slice)
model.eqImport = Constraint(model.mImport, rule = lambda model, c, dst, y, s : model.vImportTot[c,dst,y,s]  ==  sum(sum(sum(((model.pTradeIrEff[t1,src,dst,y,sp]*model.vTradeIr[t1,c,src,dst,y,sp]) if (t1,c,src,dst,y,sp) in model.mvTradeIr else 0) for src in model.region if (t1,src,dst) in model.mTradeRoutes) for t1 in model.trade if (t1,c) in model.mTradeComm) for sp in model.slice if (c,s,sp) in model.mCommSliceOrParent)+sum(sum((model.vImportRow[i,c,dst,y,sp] if (i,c,dst,y,sp) in model.mImportRow else 0) for i in model.imp if (i,c) in model.mImpComm) for sp in model.slice if (c,s,sp) in model.mCommSliceOrParent));
# eqExport(comm, src, year, slice)$mExport(comm, src, year, slice)
model.eqExport = Constraint(model.mExport, rule = lambda model, c, src, y, s : model.vExportTot[c,src,y,s]  ==  sum(sum(sum((model.vTradeIr[t1,c,src,dst,y,sp] if (t1,c,src,dst,y,sp) in model.mvTradeIr else 0) for dst in model.region if (t1,src,dst) in model.mTradeRoutes) for t1 in model.trade if (t1,c) in model.mTradeComm) for sp in model.slice if (c,s,sp) in model.mCommSliceOrParent)+sum(sum((model.vExportRow[e,c,src,y,sp] if (e,c,src,y,sp) in model.mExportRow else 0) for e in model.expp if (e,c) in model.mExpComm) for sp in model.slice if (c,s,sp) in model.mCommSliceOrParent));
...
model.eqTradeCap = Constraint(model.mTradeSpan, rule = lambda model, t1, y : model.vTradeCap[t1,y]  ==  model.pTradeStock[t1,y]+sum(model.pPeriodLen[yp]*model.vTradeNewCap[t1,yp] for yp in model.year if ((t1,yp) in model.mTradeNew and model.ordYear[y] >= model.ordYear[yp] and (model.ordYear[y]<model.pTradeOlife[t1]+model.ordYear[yp] or t1 in model.mTradeOlifeInf))));
...
model.eqTradeEac = Constraint(model.mTradeEac, rule = lambda model, t1, r, y : model.vTradeEac[t1,r,y]  ==  sum(model.pTradeEac[t1,r,yp]*model.pPeriodLen[yp]*model.vTradeNewCap[t1,yp] for yp in model.year if ((t1,yp) in model.mTradeNew and model.ordYear[y] >= model.ordYear[yp] and (model.ordYear[y]<model.pTradeOlife[t1]+model.ordYear[yp] or t1 in model.mTradeOlifeInf))));
# eqTradeIrAInp(trade, comm, region, year, slice)$mvTradeIrAInp(trade, comm, region, year, slice)
model.eqTradeIrAInp = Constraint(model.mvTradeIrAInp, rule = lambda model, t1, c, r, y, s : model.vTradeIrAInp[t1,c,r,y,s]  ==  sum(model.pTradeIrCsrc2Ainp[t1,c,r,dst,y,s]*sum(model.vTradeIr[t1,cp,r,dst,y,s] for cp in model.comm if (t1,cp) in model.mTradeComm) for dst in model.region if (t1,c,r,dst,y,s) in model.mTradeIrCsrc2Ainp)+sum(model.pTradeIrCdst2Ainp[t1,c,src,r,y,s]*sum(model.vTradeIr[t1,cp,src,r,y,s] for cp in model.comm if (t1,cp) in model.mTradeComm) for src in model.region if (t1,c,src,r,y,s) in model.mTradeIrCdst2Ainp));
# eqTradeIrAOut(trade, comm, region, year, slice)$mvTradeIrAOut(trade, comm, region, year, slice)
model.eqTradeIrAOut = Constraint(model.mvTradeIrAOut, rule = lambda model, t1, c, r, y, s : model.vTradeIrAOut[t1,c,r,y,s]  ==  sum(model.pTradeIrCsrc2Aout[t1,c,r,dst,y,s]*sum(model.vTradeIr[t1,cp,r,dst,y,s] for cp in model.comm if (t1,cp) in model.mTradeComm) for dst in model.region if (t1,c,r,dst,y,s) in model.mTradeIrCsrc2Aout)+sum(model.pTradeIrCdst2Aout[t1,c,src,r,y,s]*sum(model.vTradeIr[t1,cp,src,r,y,s] for cp in model.comm if (t1,cp) in model.mTradeComm) for src in model.region if (t1,c,src,r,y,s) in model.mTradeIrCdst2Aout));
# eqTradeIrAInpTot(comm, region, year, slice)$mvTradeIrAInpTot(comm, region, year, slice)
model.eqTradeIrAInpTot = Constraint(model.mvTradeIrAInpTot, rule = lambda model, c, r, y, s : model.vTradeIrAInpTot[c,r,y,s]  ==  model.pSliceWeight[s]*sum(model.vTradeIrAInp[t1,c,r,y,sp] for t1 in model.trade for sp in model.slice if ((c,s,sp) in model.mCommSliceOrParent and (t1,c,r,y,sp) in model.mvTradeIrAInp)));
# eqTradeIrAOutTot(comm, region, year, slice)$mvTradeIrAOutTot(comm, region, year, slice)
model.eqTradeIrAOutTot = Constraint(model.mvTradeIrAOutTot, rule = lambda model, c, r, y, s : model.vTradeIrAOutTot[c,r,y,s]  ==  model.pSliceWeight[s]*sum(model.vTradeIrAOut[t1,c,r,y,sp] for t1 in model.trade for sp in model.slice if ((c,s,sp) in model.mCommSliceOrParent and (t1,c,r,y,sp) in model.mvTradeIrAOut)));
...
model.eqBal = Constraint(model.mvBalance, rule = lambda model, c, r, y, s : model.vBalance[c,r,y,s]*model.pSliceWeight[s]  ==  (model.vOutTot[c,r,y,s] if (c,r,y,s) in model.mvOutTot else 0)-(model.vInpTot[c,r,y,s] if (c,r,y,s) in model.mvInpTot else 0));
# eqOutTot(comm, region, year, slice)$mvOutTot(comm, region, year, slice)
model.eqOutTot = Constraint(model.mvOutTot, rule = lambda model, c, r, y, s : model.vOutTot[c,r,y,s]  ==  model.pSliceWeight[s]*(model.vDummyImport[c,r,y,s] if (c,r,y,s) in model.mDummyImport else 0)+(model.vSupOutTot[c,r,y,s] if (c,r,y,s) in model.mSupOutTot else 0)+(model.vEmsFuelTot[c,r,y,s] if (c,r,y,s) in model.mEmsFuelTot else 0)+model.pSliceWeight[s]*(model.vAggOutTot[c,r,y,s] if (c,r,y,s) in model.mAggOut else 0)+(model.vTechOutTot[c,r,y,s] if (c,r,y,s) in model.mTechOutTot else 0)+(model.vStorageOutTot[c,r,y,s] if (c,r,y,s) in model.mStorageOutTot else 0)+(model.vImportTot[c,r,y,s] if (c,r,y,s) in model.mImport else 0)+(model.vTradeIrAOutTot[c,r,y,s] if (c,r,y,s) in model.mvTradeIrAOutTot else 0)+model.pSliceWeight[s]*(sum(model.vOut2Lo[c,r,y,sp,s] for sp in model.slice if ((sp,s) in model.mSliceParentChild and (c,r,y,sp,s) in model.mvOut2Lo)) if (c,r,y,s) in model.mOutSub else 0));
# eqOut2Lo(comm, region, year, slice)$mOut2Lo(comm, region, year, slice)
model.eqOut2Lo = Constraint(model.mOut2Lo, rule = lambda model, c, r, y, s : sum(model.vOut2Lo[c,r,y,s,sp] for sp in model.slice if (c,r,y,s,sp) in model.mvOut2Lo)  ==  (model.vSupOutTot[c,r,y,s] if (c,r,y,s) in model.mSupOutTot else 0)+(model.vEmsFuelTot[c,r,y,s] if (c,r,y,s) in model.mEmsFuelTot else 0)+model.pSliceWeight[s]*(model.vAggOutTot[c,r,y,s] if (c,r,y,s) in model.mAggOut else 0)+(model.vTechOutTot[c,r,y,s] if (c,r,y,s) in model.mTechOutTot else 0)+(model.vStorageOutTot[c,r,y,s] if (c,r,y,s) in model.mStorageOutTot else 0)+(model.vImportTot[c,r,y,s] if (c,r,y,s) in model.mImport else 0)+(model.vTradeIrAOutTot[c,r,y,s] if (c,r,y,s) in model.mvTradeIrAOutTot else 0));
# eqInpTot(comm, region, year, slice)$mvInpTot(comm, region, year, slice)
model.eqInpTot = Constraint(model.mvInpTot, rule = lambda model, c, r, y, s : model.vInpTot[c,r,y,s]  ==  model.pSliceWeight[s]*(model.vDemInp[c,r,y,s] if (c,r,y,s) in model.mvDemInp else 0)+(model.vDummyExport[c,r,y,s] if (c,r,y,s) in model.mDummyExport else 0)+(model.vTechInpTot[c,r,y,s] if (c,r,y,s) in model.mTechInpTot else 0)+(model.vStorageInpTot[c,r,y,s] if (c,r,y,s) in model.mStorageInpTot else 0)+(model.vExportTot[c,r,y,s] if (c,r,y,s) in model.mExport else 0)+(model.vTradeIrAInpTot[c,r,y,s] if (c,r,y,s) in model.mvTradeIrAInpTot else 0)+model.pSliceWeight[s]*(sum(model.vInp2Lo[c,r,y,sp,s] for sp in model.slice if ((sp,s) in model.mSliceParentChild and (c,r,y,sp,s) in model.mvInp2Lo)) if (c,r,y,s) in model.mInpSub else 0));
# eqInp2Lo(comm, region, year, slice)$mInp2Lo(comm, region, year, slice)
model.eqInp2Lo = Constraint(model.mInp2Lo, rule = lambda model, c, r, y, s : sum(model.vInp2Lo[c,r,y,s,sp] for sp in model.slice if (c,r,y,s,sp) in model.mvInp2Lo)  ==  (model.vTechInpTot[c,r,y,s] if (c,r,y,s) in model.mTechInpTot else 0)+(model.vStorageInpTot[c,r,y,s] if (c,r,y,s) in model.mStorageInpTot else 0)+(model.vExportTot[c,r,y,s] if (c,r,y,s) in model.mExport else 0)+(model.vTradeIrAInpTot[c,r,y,s] if (c,r,y,s) in model.mvTradeIrAInpTot else 0));
# eqSupOutTot(comm, region, year, slice)$mSupOutTot(comm, region, year, slice)
model.eqSupOutTot = Constraint(model.mSupOutTot, rule = lambda model, c, r, y, s : model.vSupOutTot[c,r,y,s]  ==  model.pSliceWeight[s]*sum(sum(model.vSupOut[s1,c,r,y,sp] for sp in model.slice if ((c,s,sp) in model.mCommSliceOrParent and (s1,c,r,y,sp) in model.mSupAva)) for s1 in model.sup if (s1,c) in model.mSupComm));
# eqTechInpTot(comm, region, year, slice)$mTechInpTot(comm, region, year, slice)
model.eqTechInpTot = Constraint(model.mTechInpTot, rule = lambda model, c, r, y, s : model.vTechInpTot[c,r,y,s]  ==  model.pSliceWeight[s]*sum(sum((model.vTechInp[t,c,r,y,sp] if (t,c,r,y,sp) in model.mvTechInp else 0) for sp in model.slice if ((t,sp) in model.mTechSlice and (c,s,sp) in model.mCommSliceOrParent)) for t in model.tech if (t,c) in model.mTechInpComm)+model.pSliceWeight[s]*sum(sum((model.vTechAInp[t,c,r,y,sp] if (t,c,r,y,sp) in model.mvTechAInp else 0) for sp in model.slice if ((t,sp) in model.mTechSlice and (c,s,sp) in model.mCommSliceOrParent)) for t in model.tech if (t,c) in model.mTechAInp));
# eqTechOutTot(comm, region, year, slice)$mTechOutTot(comm, region, year, slice)
model.eqTechOutTot = Constraint(model.mTechOutTot, rule = lambda model, c, r, y, s : model.vTechOutTot[c,r,y,s]  ==  model.pSliceWeight[s]*sum(sum((model.vTechOut[t,c,r,y,sp] if (t,c,r,y,sp) in model.mvTechOut else 0) for sp in model.slice if ((t,sp) in model.mTechSlice and (c,s,sp) in model.mCommSliceOrParent)) for t in model.tech if (t,c) in model.mTechOutComm)+model.pSliceWeight[s]*sum(sum((model.vTechAOut[t,c,r,y,sp] if (t,c,r,y,sp) in model.mvTechAOut else 0) for sp in model.slice if ((t,sp) in model.mTechSlice and (c,s,sp) in model.mCommSliceOrParent)) for t in model.tech if (t,c) in model.mTechAOut));
# eqStorageInpTot(comm, region, year, slice)$mStorageInpTot(comm, region, year, slice)
model.eqStorageInpTot = Constraint(model.mStorageInpTot, rule = lambda model, c, r, y, s : model.vStorageInpTot[c,r,y,s]  ==  model.pSliceWeight[s]*sum(model.vStorageInp[st1,c,r,y,s] for st1 in model.stg if (st1,c,r,y,s) in model.mvStorageStore)+model.pSliceWeight[s]*sum(model.vStorageAInp[st1,c,r,y,s] for st1 in model.stg if (st1,c,r,y,s) in model.mvStorageAInp));
# eqStorageOutTot(comm, region, year, slice)$mStorageOutTot(comm, region, year, slice)
model.eqStorageOutTot = Constraint(model.mStorageOutTot, rule = lambda model, c, r, y, s : model.vStorageOutTot[c,r,y,s]  ==  model.pSliceWeight[s]*sum(model.vStorageOut[st1,c,r,y,s] for st1 in model.stg if (st1,c,r,y,s) in model.mvStorageStore)+model.pSliceWeight[s]*sum(model.vStorageAOut[st1,c,r,y,s] for st1 in model.stg if (st1,c,r,y,s) in model.mvStorageAOut));
# eqCost(region, year)$mvTotalCost(region, year)
model.eqCost = Constraint(model.mvTotalCost, rule = lambda model, r, y : model.vTotalCost[r,y]  ==  sum(model.vTechEac[t,r,y] for t in model.tech if (t,r,y) in model.mTechEac)+sum(model.vTechOMCost[t,r,y] for t in model.tech if (t,r,y) in model.mTechOMCost)+sum(model.vSupCost[s1,r,y] for s1 in model.sup if (s1,r,y) in model.mvSupCost)+sum(model.pDummyImportCost[c,r,y,s]*model.pSliceWeight[s]*model.vDummyImport[c,r,y,s] for c in model.comm for s in model.slice if (c,r,y,s) in model.mDummyImport)+sum(model.pDummyExportCost[c,r,y,s]*model.pSliceWeight[s]*model.vDummyExport[c,r,y,s] for c in model.comm for s in model.slice if (c,r,y,s) in model.mDummyExport)+sum(model.vTaxCost[c,r,y] for c in model.comm if (c,r,y) in model.mTaxCost)-sum(model.vSubsCost[c,r,y] for c in model.comm if (c,r,y) in model.mSubCost)+sum(model.vStorageOMCost[st1,r,y] for st1 in model.stg if (st1,r,y) in model.mStorageOMCost)+sum(model.vStorageEac[st1,r,y] for st1 in model.stg if (st1,r,y) in model.mStorageEac)+(model.vTradeCost[r,y] if (r,y) in model.mvTradeCost else 0)+(model.vTotalUserCosts[r,y] if (r,y) in model.mvTotalUserCosts else 0));
# eqTaxCost(comm, region, year)$mTaxCost(comm, region, year)
model.eqTaxCost = Constraint(model.mTaxCost, rule = lambda model, c, r, y : model.vTaxCost[c,r,y]  ==  sum(model.pTaxCostOut[c,r,y,s]*model.vOutTot[c,r,y,s] for s in model.slice if ((c,r,y,s) in model.mvOutTot and (c,s) in model.mCommSlice))+sum(model.pTaxCostInp[c,r,y,s]*model.vInpTot[c,r,y,s] for s in model.slice if ((c,r,y,s) in model.mvInpTot and (c,s) in model.mCommSlice))+sum(model.pTaxCostBal[c,r,y,s]*model.pSliceWeight[s]*model.vBalance[c,r,y,s] for s in model.slice if ((c,r,y,s) in model.mvBalance and (c,s) in model.mCommSlice)));
# eqSubsCost(comm, region, year)$mSubCost(comm, region, year)
model.eqSubsCost = Constraint(model.mSubCost, rule = lambda model, c, r, y : model.vSubsCost[c,r,y]  ==  sum(model.pSubCostOut[c,r,y,s]*model.vOutTot[c,r,y,s] for s in model.slice if ((c,r,y,s) in model.mvOutTot and (c,s) in model.mCommSlice))+sum(model.pSubCostInp[c,r,y,s]*model.vInpTot[c,r,y,s] for s in model.slice if ((c,r,y,s) in model.mvInpTot and (c,s) in model.mCommSlice))+sum(model.pSubCostBal[c,r,y,s]*model.pSliceWeight[s]*model.vBalance[c,r,y,s] for s in model.slice if ((c,r,y,s) in model.mvBalance and (c,s) in model.mCommSlice)));
# eqObjective
