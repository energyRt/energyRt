...
model.eqImport = Constraint(model.mImport, rule = lambda model, c, dst, y, s : model.vImport[c,dst,y,s]  ==  (sum(sum(sum(((model.pTradeIrEff[t1,src,dst,y,sp]*model.vTradeIr[t1,c,src,dst,y,sp]) if (t1,c,src,dst,y,sp) in model.mvTradeIr else 0) for src in model.region if (t1,src,dst) in model.mTradeRoutes) for t1 in model.trade if (t1,c,dst,y,s,sp) in model.mImportIrSubSliceTrd) for sp in model.slice if (c,dst,y,s,sp) in model.mImportIrSubSlice) if (c,dst,y,s) in model.mImportIrSub else 0)+(sum(sum((model.vImportRow[i,c,dst,y,sp] if (i,c,dst,y,sp) in model.mImportRow else 0) for i in model.imp if (i,c) in model.mImpComm) for sp in model.slice if (c,dst,y,s,sp) in model.mImportRowSubSlice) if (c,dst,y,s) in model.mImportRowSub else 0));
# eqExport(comm, src, year, slice)$mExport(comm, src, year, slice)
model.eqExport = Constraint(model.mExport, rule = lambda model, c, src, y, s : model.vExport[c,src,y,s]  ==  (sum(sum(sum((model.vTradeIr[t1,c,src,dst,y,sp] if (t1,c,src,dst,y,sp) in model.mvTradeIr else 0) for dst in model.region if (t1,src,dst) in model.mTradeRoutes) for t1 in model.trade if (t1,c,src,y,s,sp) in model.mExportIrSubSliceTrd) for sp in model.slice if (c,src,y,s,sp) in model.mExportIrSubSlice) if (c,src,y,s) in model.mExportIrSub else 0)+(sum(sum((model.vExportRow[e,c,src,y,sp] if (e,c,src,y,sp) in model.mExportRow else 0) for e in model.expp if (e,c) in model.mExpComm) for sp in model.slice if (c,src,y,s,sp) in model.mExportRowSubSlice) if (c,src,y,s) in model.mExportRowSub else 0));
...
model.eqSupOutTot = Constraint(model.mSupOutTot, rule = lambda model, c, r, y, s : model.vSupOutTot[c,r,y,s]  ==  sum(sum(model.vSupOut[s1,c,r,y,sp] for sp in model.slice if (s1,c,r,y,sp) in model.mSupAva) for s1 in model.sup if (s1,c) in model.mSupComm));
