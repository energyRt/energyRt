...
model.eqTechEac = Constraint(model.mTechEac, rule = lambda model, t, r, y : model.vTechEac[t,r,y]  ==  sum(model.pYearFraction[y]*model.pTechEac[t,r,yp]*model.pPeriodLen[yp]*(model.vTechNewCap[t,r,yp]-sum(model.vTechRetiredNewCap[t,r,yp,ye] for ye in model.year if ((t,r,yp,ye) in model.mvTechRetiredNewCap and model.ordYear[y] >= model.ordYear[ye]))) for yp in model.year if ((t,r,yp) in model.mTechNew and model.ordYear[y] >= model.ordYear[yp] and (model.ordYear[y]<model.pTechOlife[t,r]+model.ordYear[yp] or (t,r) in model.mTechOlifeInf))));
# eqTechInv(tech, region, year)$mTechInv(tech, region, year)
model.eqTechInv = Constraint(model.mTechInv, rule = lambda model, t, r, y : model.vTechInv[t,r,y]  ==  model.pYearFraction[y]*model.pTechInvcost[t,r,y]*model.vTechNewCap[t,r,y]);
# eqTechOMCost(tech, region, year)$mTechOMCost(tech, region, year)
model.eqTechOMCost = Constraint(model.mTechOMCost, rule = lambda model, t, r, y : model.vTechOMCost[t,r,y]  ==  model.pYearFraction[y]*model.pTechFixom[t,r,y]*model.vTechCap[t,r,y]+sum(model.pTechVarom[t,r,y,s]*model.vTechAct[t,r,y,s]+sum(model.pTechCvarom[t,c,r,y,s]*model.vTechInp[t,c,r,y,s] for c in model.comm if (t,c) in model.mTechInpComm)+sum(model.pTechCvarom[t,c,r,y,s]*model.vTechOut[t,c,r,y,s] for c in model.comm if (t,c) in model.mTechOutComm)+sum(model.pTechAvarom[t,c,r,y,s]*model.vTechAOut[t,c,r,y,s] for c in model.comm if (t,c,r,y,s) in model.mvTechAOut)+sum(model.pTechAvarom[t,c,r,y,s]*model.vTechAInp[t,c,r,y,s] for c in model.comm if (t,c,r,y,s) in model.mvTechAInp) for s in model.slice if (t,s) in model.mTechSlice));
...
model.eqSupTotal = Constraint(model.mvSupReserve, rule = lambda model, s1, c, r : model.vSupReserve[s1,c,r]  ==  sum((model.pPeriodLen[y]*model.vSupOut[s1,c,r,y,s]) / (model.pYearFraction[y]) for y in model.year for s in model.slice if (s1,c,r,y,s) in model.mSupAva));
...
model.eqStorageEac = Constraint(model.mStorageEac, rule = lambda model, st1, r, y : model.vStorageEac[st1,r,y]  ==  sum(model.pYearFraction[y]*model.pStorageEac[st1,r,yp]*model.pPeriodLen[yp]*model.vStorageNewCap[st1,r,yp] for yp in model.year if ((st1,r,yp) in model.mStorageNew and model.ordYear[y] >= model.ordYear[yp] and ((st1,r) in model.mStorageOlifeInf or model.ordYear[y]<model.pStorageOlife[st1,r]+model.ordYear[yp]) and model.pStorageInvcost[st1,r,yp] != 0)));
# eqStorageCost(stg, region, year)$mStorageOMCost(stg, region, year)
model.eqStorageCost = Constraint(model.mStorageOMCost, rule = lambda model, st1, r, y : model.vStorageOMCost[st1,r,y]  ==  model.pYearFraction[y]*model.pStorageFixom[st1,r,y]*model.vStorageCap[st1,r,y]+sum(sum(model.pStorageCostInp[st1,r,y,s]*model.vStorageInp[st1,c,r,y,s]+model.pStorageCostOut[st1,r,y,s]*model.vStorageOut[st1,c,r,y,s]+model.pStorageCostStore[st1,r,y,s]*model.vStorageStore[st1,c,r,y,s] for s in model.slice if (c,s) in model.mCommSlice) for c in model.comm if (st1,c) in model.mStorageComm));
...
model.eqTradeEac = Constraint(model.mTradeEac, rule = lambda model, t1, r, y : model.vTradeEac[t1,r,y]  ==  sum(model.pYearFraction[y]*model.pTradeEac[t1,r,yp]*model.pPeriodLen[yp]*model.vTradeNewCap[t1,yp] for yp in model.year if ((t1,yp) in model.mTradeNew and model.ordYear[y] >= model.ordYear[yp] and (model.ordYear[y]<model.pTradeOlife[t1]+model.ordYear[yp] or t1 in model.mTradeOlifeInf))));
...
model.eqOutTot = Constraint(model.mvOutTot, rule = lambda model, c, r, y, s : model.vOutTot[c,r,y,s]*model.pYearFraction[y]  ==  (model.vDummyImport[c,r,y,s] if (c,r,y,s) in model.mDummyImport else 0)+(model.vSupOutTot[c,r,y,s] if (c,r,y,s) in model.mSupOutTot else 0)+(model.vEmsFuelTot[c,r,y,s] if (c,r,y,s) in model.mEmsFuelTot else 0)+(model.vAggOut[c,r,y,s] if (c,r,y,s) in model.mAggOut else 0)+(model.vTechOutTot[c,r,y,s] if (c,r,y,s) in model.mTechOutTot else 0)+(model.vStorageOutTot[c,r,y,s] if (c,r,y,s) in model.mStorageOutTot else 0)+(model.vImport[c,r,y,s] if (c,r,y,s) in model.mImport else 0)+(model.vTradeIrAOutTot[c,r,y,s] if (c,r,y,s) in model.mvTradeIrAOutTot else 0)+(sum(model.vOut2Lo[c,r,y,sp,s] for sp in model.slice if ((sp,s) in model.mSliceParentChild and (c,r,y,sp,s) in model.mvOut2Lo)) if (c,r,y,s) in model.mOutSub else 0));
...
model.eqSupOutTot = Constraint(model.mSupOutTot, rule = lambda model, c, r, y, s : model.vSupOutTot[c,r,y,s]  ==  sum(sum(model.vSupOut[s1,c,r,y,sp] for sp in model.slice if ((c,s,sp) in model.mCommSliceOrParent and (s1,c,r,y,sp) in model.mSupAva)) for s1 in model.sup if (s1,c) in model.mSupComm));
# eqTechInpTot(comm, region, year, slice)$mTechInpTot(comm, region, year, slice)
model.eqTechInpTot = Constraint(model.mTechInpTot, rule = lambda model, c, r, y, s : model.vTechInpTot[c,r,y,s]  ==  sum(sum((model.vTechInp[t,c,r,y,sp] if (t,c,r,y,sp) in model.mvTechInp else 0) for sp in model.slice if ((t,c,s,sp) in model.mTechCommSliceSliceP)) for t in model.tech if (t,c) in model.mTechInpComm)+sum(sum((model.vTechAInp[t,c,r,y,sp] if (t,c,r,y,sp) in model.mvTechAInp else 0) for sp in model.slice if ((t,c,s,sp) in model.mTechCommSliceSliceP)) for t in model.tech if (t,c) in model.mTechAInp));
# eqTechOutS(tech, comm, region, year, slice)$mvTechOutS(tech, comm, region, year, slice)
model.eqTechOutS = Constraint(model.mvTechOutS, rule = lambda model, t, c, r, y, s : model.vTechOutS[t,c,r,y,s]  ==  sum((model.vTechOut[t,c,r,y,sp] if (t,c,r,y,sp) in model.mvTechOut else 0) for sp in model.slice if ((t,c,s,sp) in model.mTechCommSliceSliceP)));
# eqTechAOutS(tech, comm, region, year, slice)$mvTechAOutS(tech, comm, region, year, slice)
model.eqTechAOutS = Constraint(model.mvTechAOutS, rule = lambda model, t, c, r, y, s : model.vTechAOutS[t,c,r,y,s]  ==  sum((model.vTechAOut[t,c,r,y,sp] if (t,c,r,y,sp) in model.mvTechAOut else 0) for sp in model.slice if ((t,c,s,sp) in model.mTechCommSliceSliceP)));
# eqTechOutTot(comm, region, year, slice)$mTechOutTot(comm, region, year, slice)
model.eqTechOutTot = Constraint(model.mTechOutTot, rule = lambda model, c, r, y, s : model.vTechOutTot[c,r,y,s]  ==  sum((model.vTechOutS[t,c,r,y,s] if (t,c,r,y,s) in model.mvTechOutS else 0) for t in model.tech if (t,c) in model.mTechOutComm)+sum((model.vTechAOutS[t,c,r,y,s] if (t,c,r,y,s) in model.mvTechAOutS else 0) for t in model.tech if (t,c) in model.mTechAOut));
