...
model.eqTechRampUp = Constraint(model.mTechRampUp, rule = lambda model, t, r, y, s : (model.vTechAct[t,r,y,s]) / (model.pSliceShare[s])-sum((model.vTechAct[t,r,y,sp]) / (model.pSliceShare[sp]) for sp in model.slice if (((t in model.mTechFullYear and (sp,s) in model.mSliceFYearNext) or (not((t in model.mTechFullYear)) and (sp,s) in model.mSliceNext)) and (t,r,y,sp) in model.mvTechAct)) <=  (model.pSliceShare[s]*365*24*model.pYearFraction[y]*model.pYearFraction[y]*model.pTechCap2act[t]*model.vTechCap[t,r,y]) / (model.pTechRampUp[t,r,y,s]));
# eqTechRampDown(tech, region, year, slice)$mTechRampDown(tech, region, year, slice)
model.eqTechRampDown = Constraint(model.mTechRampDown, rule = lambda model, t, r, y, s : sum((model.vTechAct[t,r,y,sp]) / (model.pSliceShare[sp]) for sp in model.slice if (((t in model.mTechFullYear and (sp,s) in model.mSliceFYearNext) or (not((t in model.mTechFullYear)) and (sp,s) in model.mSliceNext)) and (t,r,y,sp) in model.mvTechAct))-(model.vTechAct[t,r,y,s]) / (model.pSliceShare[s]) <=  (model.pSliceShare[s]*365*24*model.pYearFraction[y]*model.pYearFraction[y]*model.pTechCap2act[t]*model.vTechCap[t,r,y]) / (model.pTechRampDown[t,r,y,s]));
