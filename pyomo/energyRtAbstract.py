...
model.eqOutTot = Constraint(model.mvOutTot, rule = lambda model, c, r, y, s : model.vOutTot[c,r,y,s]  ==  model.pSliceWeight[s]*(model.vDummyImport[c,r,y,s] if (c,r,y,s) in model.mDummyImport else 0)+(model.vSupOutTot[c,r,y,s] if (c,r,y,s) in model.mSupOutTot else 0)+(model.vEmsFuelTot[c,r,y,s] if (c,r,y,s) in model.mEmsFuelTot else 0)+model.pSliceWeight[s]*(model.vAggOut[c,r,y,s] if (c,r,y,s) in model.mAggOut else 0)+(model.vTechOutTot[c,r,y,s] if (c,r,y,s) in model.mTechOutTot else 0)+(model.vStorageOutTot[c,r,y,s] if (c,r,y,s) in model.mStorageOutTot else 0)+(model.vImport[c,r,y,s] if (c,r,y,s) in model.mImport else 0)+(model.vTradeIrAOutTot[c,r,y,s] if (c,r,y,s) in model.mvTradeIrAOutTot else 0)+model.pSliceWeight[s]*(sum(model.vOut2Lo[c,r,y,sp,s] for sp in model.slice if ((sp,s) in model.mSliceParentChild and (c,r,y,sp,s) in model.mvOut2Lo)) if (c,r,y,s) in model.mOutSub else 0));
# eqOut2Lo(comm, region, year, slice)$mOut2Lo(comm, region, year, slice)
model.eqOut2Lo = Constraint(model.mOut2Lo, rule = lambda model, c, r, y, s : sum(model.vOut2Lo[c,r,y,s,sp] for sp in model.slice if (c,r,y,s,sp) in model.mvOut2Lo)  ==  (model.vSupOutTot[c,r,y,s] if (c,r,y,s) in model.mSupOutTot else 0)+(model.vEmsFuelTot[c,r,y,s] if (c,r,y,s) in model.mEmsFuelTot else 0)+model.pSliceWeight[s]*(model.vAggOut[c,r,y,s] if (c,r,y,s) in model.mAggOut else 0)+(model.vTechOutTot[c,r,y,s] if (c,r,y,s) in model.mTechOutTot else 0)+(model.vStorageOutTot[c,r,y,s] if (c,r,y,s) in model.mStorageOutTot else 0)+(model.vImport[c,r,y,s] if (c,r,y,s) in model.mImport else 0)+(model.vTradeIrAOutTot[c,r,y,s] if (c,r,y,s) in model.mvTradeIrAOutTot else 0));
# eqInpTot(comm, region, year, slice)$mvInpTot(comm, region, year, slice)
model.eqInpTot = Constraint(model.mvInpTot, rule = lambda model, c, r, y, s : model.vInpTot[c,r,y,s]  ==  model.pSliceWeight[s]*(model.vDemInp[c,r,y,s] if (c,r,y,s) in model.mvDemInp else 0)+(model.vDummyExport[c,r,y,s] if (c,r,y,s) in model.mDummyExport else 0)+(model.vTechInpTot[c,r,y,s] if (c,r,y,s) in model.mTechInpTot else 0)+(model.vStorageInpTot[c,r,y,s] if (c,r,y,s) in model.mStorageInpTot else 0)+(model.vExport[c,r,y,s] if (c,r,y,s) in model.mExport else 0)+(model.vTradeIrAInpTot[c,r,y,s] if (c,r,y,s) in model.mvTradeIrAInpTot else 0)+model.pSliceWeight[s]*(sum(model.vInp2Lo[c,r,y,sp,s] for sp in model.slice if ((sp,s) in model.mSliceParentChild and (c,r,y,sp,s) in model.mvInp2Lo)) if (c,r,y,s) in model.mInpSub else 0));
