...
model.eqTechCap = Constraint(model.mTechSpan, rule = lambda model, t, r, y : model.vTechCap[t,r,y]  ==  model.pTechStock[t,r,y]-(model.vTechRetiredStock[t,r,y] if (t,r,y) in model.mvTechRetiredStock else 0)+sum(model.vTechNewCap[t,r,yp]-sum(model.vTechRetiredNewCap[t,r,yp,ye] for ye in model.year if ((t,r,yp,ye) in model.mvTechRetiredNewCap and model.ordYear[y] >= model.ordYear[ye])) for yp in model.year if ((t,r,yp) in model.mTechNew and model.ordYear[y] >= model.ordYear[yp] and (model.ordYear[y]<model.pTechOlife[t,r]+model.ordYear[yp] or (t,r) in model.mTechOlifeInf))));
