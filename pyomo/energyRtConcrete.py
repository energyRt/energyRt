...
model.eqStorageAInp = Constraint(mvStorageAInp, rule = lambda model, st1, c, r, y, s : model.vStorageAInp[st1,c,r,y,s]  ==  sum(((pStorageStg2AInp.get((st1,c,r,y,s))*model.vStorageStore[st1,cp,r,y,s]) if (st1,c,r,y,s) in mStorageStg2AInp else 0)+((pStorageInp2AInp.get((st1,c,r,y,s))*model.vStorageInp[st1,cp,r,y,s]) if (st1,c,r,y,s) in mStorageInp2AInp else 0)+((pStorageOut2AInp.get((st1,c,r,y,s))*model.vStorageOut[st1,cp,r,y,s]) if (st1,c,r,y,s) in mStorageOut2AInp else 0)+((pStorageCap2AInp.get((st1,c,r,y,s))*model.vStorageCap[st1,r,y]) if (st1,c,r,y,s) in mStorageCap2AInp else 0)+((pStorageNCap2AInp.get((st1,c,r,y,s))*model.vStorageNewCap[st1,r,y]) if (st1,c,r,y,s) in mStorageNCap2AInp else 0) for cp in comm if (st1,cp) in mStorageComm));
if verbose: print(datetime.datetime.now().strftime("%H:%M:%S"), " (", round(time.time() - seconds, 2), " s)", sep = "")
if verbose: print("eqStorageAOut ", end = "")
# eqStorageAOut(stg, comm, region, year, slice)$mvStorageAOut(stg, comm, region, year, slice)
model.eqStorageAOut = Constraint(mvStorageAOut, rule = lambda model, st1, c, r, y, s : model.vStorageAOut[st1,c,r,y,s]  ==  sum(((pStorageStg2AOut.get((st1,c,r,y,s))*model.vStorageStore[st1,cp,r,y,s]) if (st1,c,r,y,s) in mStorageStg2AOut else 0)+((pStorageInp2AOut.get((st1,c,r,y,s))*model.vStorageInp[st1,cp,r,y,s]) if (st1,c,r,y,s) in mStorageInp2AOut else 0)+((pStorageOut2AOut.get((st1,c,r,y,s))*model.vStorageOut[st1,cp,r,y,s]) if (st1,c,r,y,s) in mStorageOut2AOut else 0)+((pStorageCap2AOut.get((st1,c,r,y,s))*model.vStorageCap[st1,r,y]) if (st1,c,r,y,s) in mStorageCap2AOut else 0)+((pStorageNCap2AOut.get((st1,c,r,y,s))*model.vStorageNewCap[st1,r,y]) if (st1,c,r,y,s) in mStorageNCap2AOut else 0) for cp in comm if (st1,cp) in mStorageComm));
if verbose: print(datetime.datetime.now().strftime("%H:%M:%S"), " (", round(time.time() - seconds, 2), " s)", sep = "")
