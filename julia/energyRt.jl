...
@constraint(model, [(t, r, y, s) in mTechRampDown], sum((vTechAct[(t,r,y,sp)]) / ((if haskey(pSliceShare, (sp)); pSliceShare[(sp)]; else pSliceShareDef; end)) for sp in slice if ((sp,s) in mSliceNext && (t,r,y,sp) in mvTechAct))-(vTechAct[(t,r,y,s)]) / ((if haskey(pSliceShare, (s)); pSliceShare[(s)]; else pSliceShareDef; end)) <=  (if haskey(pTechRampDown, (t,r,y,s)); pTechRampDown[(t,r,y,s)]; else pTechRampDownDef; end)*(if haskey(pTechCap2act, (t)); pTechCap2act[(t)]; else pTechCap2actDef; end)*vTechCap[(t,r,y)]);
