...
@constraint(model, [(t, r, y) in mTechSpan], vTechCap[(t,r,y)]  ==  (if haskey(pTechStock, (t,r,y)); pTechStock[(t,r,y)]; else pTechStockDef; end)-(if (t,r,y) in mvTechRetiredStock; vTechRetiredStock[(t,r,y)]; else 0; end;)+sum(vTechNewCap[(t,r,yp)]-sum(vTechRetiredNewCap[(t,r,yp,ye)] for ye in year if ((t,r,yp,ye) in mvTechRetiredNewCap && ordYear[(y)] >= ordYear[(ye)])) for yp in year if ((t,r,yp) in mTechNew && ordYear[(y)] >= ordYear[(yp)] && (ordYear[(y)]<(if haskey(pTechOlife, (t,r)); pTechOlife[(t,r)]; else pTechOlifeDef; end)+ordYear[(yp)] || (t,r) in mTechOlifeInf))));
