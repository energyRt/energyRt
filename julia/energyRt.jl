...
@constraint(model, [(t, r, y, s) in mTechRampUp], vTechAct[(t,r,y,s)]-sum(vTechAct[(t,r,y,sp)] for sp in slice if (((t in mTechFullYear && (sp,s) in mSliceNext) || (!((t in mTechFullYear)) && (sp,s) in mSliceFYearNext)) && (t,r,y,sp) in mvTechAct)) <=  (if haskey(pSliceShare, (s)); pSliceShare[(s)]; else pSliceShareDef; end)*(if haskey(pTechRampUp, (t,r,y,s)); pTechRampUp[(t,r,y,s)]; else pTechRampUpDef; end)*(if haskey(pTechCap2act, (t)); pTechCap2act[(t)]; else pTechCap2actDef; end)*vTechCap[(t,r,y)]);
...
@constraint(model, [(t, r, y, s) in mTechRampDown], sum(vTechAct[(t,r,y,sp)] for sp in slice if (((t in mTechFullYear && (sp,s) in mSliceNext) || (!((t in mTechFullYear)) && (sp,s) in mSliceFYearNext)) && (t,r,y,sp) in mvTechAct))-vTechAct[(t,r,y,s)] <=  (if haskey(pSliceShare, (s)); pSliceShare[(s)]; else pSliceShareDef; end)*(if haskey(pTechRampDown, (t,r,y,s)); pTechRampDown[(t,r,y,s)]; else pTechRampDownDef; end)*(if haskey(pTechCap2act, (t)); pTechCap2act[(t)]; else pTechCap2actDef; end)*vTechCap[(t,r,y)]);
