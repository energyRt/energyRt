...
@constraint(model, [(c, dst, y, s) in mImport], vImport[(c,dst,y,s)]  ==  (if (c,dst,y,s) in mImportIrSub; sum(sum(sum((if (t1,c,src,dst,y,sp) in mvTradeIr; ((if haskey(pTradeIrEff, (t1,src,dst,y,sp)); pTradeIrEff[(t1,src,dst,y,sp)]; else pTradeIrEffDef; end)*vTradeIr[(t1,c,src,dst,y,sp)]); else 0; end;) for src in region if (t1,src,dst) in mTradeRoutes) for t1 in trade if (t1,c,dst,y,s,sp) in mImportIrSubSliceTrd) for sp in slice if (c,dst,y,s,sp) in mImportIrSubSlice); else 0; end;)+(if (c,dst,y,s) in mImportRowSub; sum(sum((if (i,c,dst,y,sp) in mImportRow; vImportRow[(i,c,dst,y,sp)]; else 0; end;) for i in imp if (i,c) in mImpComm) for sp in slice if (c,dst,y,s,sp) in mImportRowSubSlice); else 0; end;));
...
@constraint(model, [(c, src, y, s) in mExport], vExport[(c,src,y,s)]  ==  (if (c,src,y,s) in mExportIrSub; sum(sum(sum((if (t1,c,src,dst,y,sp) in mvTradeIr; vTradeIr[(t1,c,src,dst,y,sp)]; else 0; end;) for dst in region if (t1,src,dst) in mTradeRoutes) for t1 in trade if (t1,c,src,y,s,sp) in mExportIrSubSliceTrd) for sp in slice if (c,src,y,s,sp) in mExportIrSubSlice); else 0; end;)+(if (c,src,y,s) in mExportRowSub; sum(sum((if (e,c,src,y,sp) in mExportRow; vExportRow[(e,c,src,y,sp)]; else 0; end;) for e in expp if (e,c) in mExpComm) for sp in slice if (c,src,y,s,sp) in mExportRowSubSlice); else 0; end;));
