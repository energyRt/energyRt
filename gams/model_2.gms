...
pStorageStoreLoss(stg, region, year, slice)
pStorageStock(stg, region, year)
pStorageOlife(stg, region)
pStorageCapUp(stg, region, year)
pStorageCapLo(stg, region, year)
...
pTradeFlowCost(trade, region, region, year, slice)
pRowExportRes(expp)
pRowExportUp(expp, region, year, slice)
pRowExportLo(expp, region, year, slice)
pRowExportPrice(expp, region, year, slice)
pRowImportRes(imp)
pRowImportUp(imp, region, year, slice)
pRowImportLo(imp, region, year, slice)
...
vTechRetirementCap(tech, region, year, year)
...
vStorageStore(stg, region, year, slice)
...
vTradeFlow(trade, region, region, year, slice)
vRowExportRes(expp)
...
eqTechSalv2(tech, region, yeare)$(mDiscountZero(region) and mMilestoneLast(yeare))..
   ...
   sum(year$(mMidMilestone(year) and mTechNew(tech, region, year) and ORD(year) + pTechOlife(tech, region) - 1 > ORD(yeare)),
    (pDiscountMultiple(region, year) /  pDiscountMultiple(region, yeare)) *
    pTechInvcost(tech, region, year) * (vTechNewCap(tech, region, year)
     - sum(yearp$(mMidMilestone(yearp) and mTechRetirement(tech)), vTechRetirementCap(tech, region, year, yearp)))  / (
      1
        + (sum(yearp$(ORD(yearp) >= ORD(year)), pDiscountMultiple(region, yearp)))
        / (pDiscountMultiple(region, yeare)
...
eqTechSalv3(tech, region, yeare)$(not(mDiscountZero(region)) and mMilestoneLast(yeare))..
   ...
   sum(year$(mMidMilestone(year) and mTechNew(tech, region, year) and ORD(year) + pTechOlife(tech, region) - 1 > ORD(yeare)),
    ...
    pTechInvcost(tech, region, year) * (vTechNewCap(tech, region, year)
      ...
      1
        + (sum(yearp$(mMidMilestone(yearp) and ORD(yearp) >= ORD(year)), pDiscountMultiple(region, yearp)))
        / (pDiscountMultiple(region, yeare)
          ) / (sum(yearn$(ORD(yeare) = ORD(yearn)), (
...
eqStorageVar(stg, region, year)$mMidMilestone(year)..
         vStorageVarom(stg, region, year)
         =e=
         sum((slice, comm)$mResComm(stg, comm),
...
eqObjective..
         vObjective =e= sum((region, year)$mMidMilestone(year), pDiscountMultiple(region, year) *
                 vCost(region, year));
* Tax
