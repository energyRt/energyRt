---
title: "energyRt: energy analysis toolbox in R"
date: "`r Sys.Date()`"
...
##### GLPK binaries for Windows:
https://sourceforge.net/projects/winglpk/  

##### Installing GLPK on Mac systems
To our best knowlege GLPK-binaries/installers are not available for Mac OSx from the developer. One way to install it is building from source. The following example is for installed from source with a standard procedure (which worked for us):   
...
glpsol  
or glpsol -v  
A response from glpsol will be an indicator of successful installation.  

GLPK is/was also included in homebrew-science installer library. 
...
### energyRt
Direct installation the development version from GitHub:   
