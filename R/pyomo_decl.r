.generate.pyomo.par <- function(param) {  
  ...
  for (tmp in param[sapply(param, function(x) x@type == 'simple')]) {
    decl <- c(decl, 
      paste0('model.', tmp@name, ' = Param(', paste0('model.', energyRt:::.removeEndSet(tmp@dimSetNames), collapse = '*'), 
  ...
  for (tmp in param[sapply(param, function(x) x@type == 'multi')]) {
    decl <- c(decl, 
      paste0('model.', tmp@name, 'Lo = Param(', paste0('model.', energyRt:::.removeEndSet(tmp@dimSetNames), collapse = '*'), ', default = ', 
