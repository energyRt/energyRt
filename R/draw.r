...
draw.technology <- function(        
  ...
  tryCatch({
    ...
    if (length(acomm) != 0) {
      ...
      aname <- c('act2ainp', 'act2aout', 'use2ainp', 'use2aout', 'cap2ainp', 'cap2aout', 'new2ainp', 'new2aout')
      acname <- c('cinp2ainp', 'cout2ainp', 'cinp2aout', 'cout2aout')
      aparam <- lapply(acomm, function(y) {
        approxim$acomm <- y
        approxim$comm <- NULL
        #tech@aeff[tech@aeff$acomm == y,, drop = FALSE]
        ...
        list(single = sng, wcomm = dbl, input = any(names(sng) %in% c('act2ainp', 
            'use2ainp', 'cap2ainp', 'new2ainp')) || any(names(dbl) %in% c('cinp2ainp', 'cout2ainp')), 
            output = any(names(sng) %in% c('act2aout', 
            'use2aout', 'cap2aout', 'new2aout')) || any(names(dbl) %in% c('cinp2aout', 'cout2aout')))
      })
      names(aparam) <- acomm
    ...
    if (nrow(ccomm) != 0 || length(ainp) != 0) {
     ...
     if (any(!is.na(ccomm$group))) {
      ...
      for(i in seq(length.out = length(ainp))) {
        ...
        # Find 
        ll <- aparam[[cmm]]
        ll$single <- ll$single[names(ll$single) %in% c('act2ainp', 'use2ainp', 'cap2ainp', 'new2ainp')]
    ...
    if (nrow(ccomm) != 0 || length(aout) != 0) {
      ...
      for(i in seq(length.out = length(aout))) {
        ...
        # Find
        ll <- aparam[[cmm]]
        ll$single <- ll$single[names(ll$single) %in% c('act2aout', 'use2aout', 'cap2aout', 'new2aout')]
