...
draw.technology <- function(        
  ...
  tech_color <- as.data.table(tech@misc$color)
  get_set <- function(x, att) {
    ...
    for(z in names(g@slots)) {
      if (g@slots[[z]] == 'data.table' & any(colnames(slot(x, z)) == att))
  ...
  tryCatch({
    ...
    if (length(acomm) != 0) {
      approxim <- list(region = region, year = year, slice = slice, comm = acomm, fullsets = TRUE)
      ...
      aparam <- lapply(acomm, function(y) {
        ...
        list(single = sng, wcomm = dbl, input = any(names(sng) %in% c('act2ainp', 
            'cap2ainp', 'ncap2ainp')) || any(names(dbl) %in% c('cinp2ainp', 'cout2ainp')), 
            output = any(names(sng) %in% c('act2aout', 
            'cap2aout', 'ncap2aout')) || any(names(dbl) %in% c('cinp2aout', 'cout2aout')))
      })
      names(aparam) <- acomm
