...
make_timeslices_table <- function(struct = list(ANNUAL = "ANNUAL"), warn = FALSE) {
  ...
  if (length(struct) == 1 && is.character(struct[[1]]) && length(struct[[1]]) == 1) {
    #
    dtf <- data.frame(share = 1, ANNUAL = struct[[1]], stringsAsFactors = FALSE)
    if (!is.null(names(struct))) colnames(dtf)[2] <- names(struct)[1]
  } else {
    dtf <- .slice_def(dtf, struct)
  }
...
.check_timeslices_table <- function(dtf) {
  ...
  if (ncol(dtf) < 2) {
    stop("time-slices data.frame should have more than two columns")
  }
  if (colnames(dtf)[ncol(dtf)] != "share") {
    stop(
      "the name of the last column of time-slices data.frame",
      " should be 'share'"
    )
  }
...
.complete_timeslices <- function(obj) {
  ...
  # depth of levels
  d <- select(dtf, -any_of(c("share", "year")))
  obj@depth <- 1:ncol(d)
  ...
  if (ncol(dtf) > 2) {
    for (i in 2:(ncol(dtf) - 1)) {
      ...
      obj@share[k + seq(along = tmp), "slice"] <- names(tmp)
      obj@share[k + seq(along = tmp), "share"] <- tmp
      k <- (k + length(tmp))
    }
  }
  ...
  tmp[obj@table[1, 1]] <- 1
  obj@structure <- lapply(1:(ncol(obj@table) - 1), function(x) names(tmp)[tmp == x])
  names(obj@structure) <- colnames(obj@table)[-ncol(obj@table)]
  ...
  if (nrow(obj@table) == 1) {
    ...
    while (i != ncol(dtf) - 1) {
      ...
      for (j in 1:obj@nSlices[i]) {
        obj@family[k + 1:l, "parent"] <- obj@share[z, "slice"]
        obj@family[k + 1:l, "child"] <- obj@share[1 + k + 1:l, "slice"]
    ...
    for (i in seq_along(obj@structure)) {
      tmp[tmp$parent %in% obj@structure[[i]], "nlev"] <- i
    }
