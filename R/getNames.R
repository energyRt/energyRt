.getUniversalNames <- function(obj, cls, regex = NULL, ignore.case = FALSE, 
  fixed = FALSE, useBytes = FALSE, invert = FALSE, ..., .returnName) {
  if (is.null(regex)) {
  ...
  if (is.null(cls)) {
    ...
    for(i in seq(along = obj@data)) {
      ...
      if (length(jj) != 0) 
        rst <- rst %>% add_row(rp = i, ob = jj, use = TRUE)
    }
    s1 <- getSlots(cls)
    s2 <- new(cls)
        ...
        if (nrow(rst) > 0) {
          ...
          # Character
           if (!(class(cnd)  %in% c("character", 'factor'))) stop(error_msg)
            for(i in seq(length.out = nrow(rst))) 
              rst[i, 'use']  <- any(grep2(cnd, slot(obj@data[[rst$rp[i]]]@data[[rst$ob[i]]], nm), FL[nm]))
            rst <- rst[rst$use, ]
          } else if (s1[nm] == "logical") {
          # Logical
            if (class(cnd) != 'logical') stop(error_msg)
            for(i in seq(length.out = nrow(rst))) {
              rst[i, 'use']  <- any(cnd == slot(obj@data[[rst$rp[i]]]@data[[rst$ob[i]]], nm), na.rm = TRUE)
              rst <- rst[rst$use, ]
            }
          ...
          # Numeric
            if (!(class(cnd) %in% c('integer', 'numeric'))) stop(error_msg)
              ...
              if (any(names(cnd) == 'le')) {
                for(i in seq(length.out = nrow(rst)))
                  rst[i, 'use'] <- any(cnd['le'] >= slot(obj@data[[rst$rp[i]]]@data[[rst$ob[i]]], nm), na.rm = TRUE)
                  rst <- rst[rst$use, ]
              }
              if (any(names(cnd) == 'l')) {       
                for(i in seq(length.out = nrow(rst)))
                  rst[i, 'use']  <- any(cnd['l'] > slot(obj@data[[rst$rp[i]]]@data[[rst$ob[i]]], nm), na.rm = TRUE)
                  rst <- rst[rst$use, ]
              }
              if (any(names(cnd) == 'e')) {       
                for(i in seq(length.out = nrow(rst)))
                  rst[i, 'use']  <- any(cnd['e'] == slot(obj@data[[rst$rp[i]]]@data[[rst$ob[i]]], nm), na.rm = TRUE)
                  rst <- rst[rst$use, ]
              }
              if (any(names(cnd) == 'ge')) {       
                for(i in seq(length.out = nrow(rst)))
                  rst[i, 'use'] <- any(cnd['ge'] <= slot(obj@data[[rst$rp[i]]]@data[[rst$ob[i]]], nm), na.rm = TRUE)
                  rst <- rst[rst$use, ]
              }
              if (any(names(cnd) == 'g')) {       
                for(i in seq(length.out = nrow(rst)))
                  rst[i, 'use']  <- any(cnd['g'] < slot(obj@data[[rst$rp[i]]]@data[[rst$ob[i]]], nm), na.rm = TRUE)
                  rst <- rst[rst$use, ]
              }
              if (any(names(cnd) == 'ne')) {       
                for(i in seq(length.out = nrow(rst)))
                  rst[i, 'use'] <- any(cnd['ne'] != slot(obj@data[[rst$rp[i]]]@data[[rst$ob[i]]], nm), na.rm = TRUE)
                  rst <- rst[rst$use, ]
              }
          ...
          # data.table
            ...
            for(nm2 in names(cnd)) {
              ...
              if (class(cnd2)  %in% c("character", 'factor')) {
                ...
                for(i in seq(length.out = nrow(rst))) 
                  rst[i, 'use']  <- any(grep2(cnd2, 
                         slot(obj@data[[rst$rp[i]]]@data[[rst$ob[i]]], nm)[[nm2]], FL2[nm2]), na.rm = TRUE)
                rst <- rst[rst$use, ]
              ...
              # Logical
                ...
                for(i in seq(length.out = nrow(rst)))
                  rst[i, 'use']  <- any(cnd == 
                    slot(obj@data[[rst$rp[i]]]@data[[rst$ob[i]]], nm)[[nm2]], na.rm = TRUE)
                rst <- rst[rst$use, ]
    ...
    if (.returnName) {
      if (nrow(rst) == 0) return(NULL)
      nn <- rep(NA, nrow(rst))
      for(i in seq(length.out = nrow(rst)))
        nn[i] <- obj@data[[rst$rp[i]]]@data[[rst$ob[i]]]@name
      return(nn)
    }
    nn <- list()
    for(i in seq(length.out = nrow(rst)))
      nn[[obj@data[[rst$rp[i]]]@data[[rst$ob[i]]]@name]] <- obj@data[[rst$rp[i]]]@data[[rst$ob[i]]]
    nn
  }
}
...
getNames <- function(obj, class = c(), regex = NULL, ...) {
  energyRt:::.getUniversalNames(obj, cls = class, regex = regex, ..., .returnName = TRUE)
}
getNames_ <- function(obj, class = c(), ...) {
  energyRt:::.getUniversalNames(obj, cls = class, regex = TRUE, ..., .returnName = TRUE)
}


getObjects <- function(obj, class = c(), regex = NULL, ...) {   
  energyRt:::.getUniversalNames(obj, cls = class, regex = regex, ..., .returnName = FALSE)
}

getObjects_ <- function(obj, class = c(), ...) {   
  energyRt:::.getUniversalNames(obj, cls = class, regex = TRUE, ..., .returnName = FALSE)
}
