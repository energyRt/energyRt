...
setMethod(".obj2modInp",
  ...
  if (nrow(dd) > 0) {
    ...
    obj@parameters[["pEmissionFactor"]] <-
      .dat2par(obj@parameters[["pEmissionFactor"]], dd)
  }
  ...
  if (nrow(dd) > 0) {
    ...
    obj@parameters[["pAggregateFactor"]] <-
      .dat2par(obj@parameters[["pAggregateFactor"]], dd)
  }
  # @limtype: mUpComm | mLoComm | mFxComm
  if (cmd@limtype == "UP") {
    obj@parameters[["mUpComm"]] <-
      .dat2par(obj@parameters[["mUpComm"]], data.frame(comm = cmd@name))
  } else if (cmd@limtype == "LO") {
    obj@parameters[["mLoComm"]] <-
      .dat2par(obj@parameters[["mLoComm"]], data.frame(comm = cmd@name))
  } else if (cmd@limtype == "FX") {
    obj@parameters[["mFxComm"]] <- .dat2par(obj@parameters[["mFxComm"]],
                                             data.frame(comm = cmd@name))
  } else {
...
setMethod(
  ...
  clean_list <- c(
    ...
    obj@parameters[["mSliceParentChild"]] <- .dat2par(
      ...
      data.frame(
        slice = as.character(approxim$slice@all_parent_child$parent),
    ...
    obj@parameters[["mSliceParentChildE"]] <- .dat2par(
      ...
      data.frame(
        slice = as.character(c(app@slice@all_slice,
                               approxim$slice@all_parent_child$parent)),
        slicep = as.character(c(app@slice@all_slice,
                                approxim$slice@all_parent_child$child)),
        stringsAsFactors = FALSE
      )
    )
    if (length(approxim$slice@misc$next_slice) != 0) {
      obj@parameters[["mSliceNext"]] <-
        .dat2par(obj@parameters[["mSliceNext"]],
                  approxim$slice@misc$next_slice)
    ...
    obj@parameters[["pSliceShare"]] <- .dat2par(
      ...
      data.frame(
        slice = approxim$slice@slice_share$slice,
    ...
    obj@parameters[["mSameSlice"]] <- .dat2par(
      ...
      data.frame(slice = app@slice@all_slice,
                 slicep = app@slice@all_slice)
    )
    obj@parameters[["mSameRegion"]] <- .dat2par(
      obj@parameters[["mSameRegion"]],
      data.frame(region = app@region,
                 regionp = app@region)
    )
    ...
    #             data.frame(value = app@yearFraction))
    # !!!
...
setMethod(
  ...
  function(obj, app, approxim) {
    ...
    stg_slice <-
      approxim$slice@slice_map[[approxim$commodity_slice_map[[stg@commodity]]]]
...
setMethod(
  ...
  function(obj, app, approxim) {
    ...
    if (length(sup@region) != 0) {
      ...
      for (sl in ss) {
        if (any(!is.na(slot(sup, sl)$region) & !(slot(sup, sl)$region %in% sup@region))) {
      ...
      obj@parameters[["mSupSpan"]] <- .dat2par(obj@parameters[["mSupSpan"]], mSupSpan)
    } else {
    ...
    obj@parameters[["mvSupReserve"]] <- .dat2par(obj@parameters[["mvSupReserve"]], mvSupReserve)
    ...
    if (nrow(sup@weather) > 0) {
      ...
      obj@parameters[["pSupWeather"]] <- .dat2par(obj@parameters[["pSupWeather"]], tmp$par)
      obj@parameters[["mSupWeatherUp"]] <- .dat2par(obj@parameters[["mSupWeatherUp"]], tmp$mapup)
      obj@parameters[["mSupWeatherLo"]] <- .dat2par(obj@parameters[["mSupWeatherLo"]], tmp$maplo)
    }
    t1 <- mSupAva[, c("sup", "region", "year")]
    t1 <- t1[!duplicated(t1), ]
...
setMethod(
  ...
  function(obj, app, approxim) {
    ...
    if (length(tech@slice) == 0) {
      ...
      tech@slice <- colnames(approxim$slice@levels)[
        max(c(approxim$slice@misc$deep[c(use_cmd, recursive = TRUE)],
              recursive = TRUE))
        ]
    }
    # Disaggregated AFS, if there is a slice level
    if (nrow(tech@afs) != 0 &&
        any(tech@afs$slice %in% names(approxim$slice@slice_map))) {
      chk <- seq_len(nrow(tech@afs))[tech@afs$slice %in%
                                       names(approxim$slice@slice_map)]
      for (cc in chk) {
    ...
    if (!is.null(invcost)) {
      ...
      obj@parameters[["mTechInv"]] <-
        .dat2par(obj@parameters[["mTechInv"]],
                  minvcost[, colnames(minvcost) != "value"])
    ...
    if (length(approxim_comm[["comm"]]) != 0) {
      ...
      obj@parameters[["pTechCact2cout"]] <- .dat2par(obj@parameters[["pTechCact2cout"]], pTechCact2cout)
    ...
    if (length(tech@upgrade.technology) != 0) {
      obj@parameters[["mTechUpgrade"]] <- .dat2par(
        ...
        data.frame(tech = rep(tech@name, length(tech@upgrade.technology)),
    ...
    if (length(cmm) != 0) {
      ...
      obj@parameters[["mTechInpComm"]] <- .dat2par(obj@parameters[["mTechInpComm"]], mTechInpComm)
    } else {
      mTechInpComm <- NULL
    }
    ...
    if (length(gpp) != 0) {
      ...
      obj@parameters[["mTechInpGroup"]] <- .dat2par(obj@parameters[["mTechInpGroup"]], mTechInpGroup)
    } else {
      mTechInpGroup <- NULL
    }
    ...
    if (length(gpp) != 0) {
      ...
      obj@parameters[["mTechOutGroup"]] <- .dat2par(obj@parameters[["mTechOutGroup"]], mTechOutGroup)
    } else {
      mTechOutGroup <- NULL
    }
    ...
    if (length(approxim_group[["group"]]) != 0) {
      ...
      obj@parameters[["pTechGinp2use"]] <- .dat2par(obj@parameters[["pTechGinp2use"]], pTechGinp2use)
    } else {
      pTechGinp2use <- NULL
    }
    ...
    if (!is.null(mTechInpComm)) {
      ...
      obj@parameters[["mvTechInp"]] <- .dat2par(obj@parameters[["mvTechInp"]], mvTechInp)
    } else {
      mvTechInp <- NULL
    }
    if (!is.null(mTechOutComm)) {
      mvTechOut <- merge0(mvTechAct, mTechOutComm, by = "tech")
      obj@parameters[["mvTechOut"]] <- .dat2par(obj@parameters[["mvTechOut"]], mvTechOut)
    } else {
      mvTechOut <- NULL
    }
    if (!is.null(mTechAInp)) {
      mvTechAInp <- merge0(mvTechAct, mTechAInp, by = "tech")
      obj@parameters[["mvTechAInp"]] <- .dat2par(obj@parameters[["mvTechAInp"]], mvTechAInp)
    } else {
      mvTechAInp <- NULL
    }
    if (!is.null(mTechAOut)) {
      mvTechAOut <- merge0(mvTechAct, mTechAOut, by = "tech")
      obj@parameters[["mvTechAOut"]] <- .dat2par(obj@parameters[["mvTechAOut"]], mvTechAOut)
    } else {
      mvTechAOut <- NULL
    }
    #### aeff
      ...
      for (i in 1:4) {
        ...
        if (nrow(yy) != 0) {
          ...
          if (nrow(tmp) > 0) {
            ...
            tmp$comm <- tmp$acomm
            tmp$acomm <- NULL
            obj@parameters[[tbl2]] <- .dat2par(obj@parameters[[tbl2]], tmp[!duplicated(tmp), ])
          }
        }
      }
    }
    ...
    if (!is.null(mTechInpGroup) && !is.null(techSingOut)) {
      ...
      obj@parameters[["meqTechGrp2Sng"]] <- .dat2par(obj@parameters[["meqTechGrp2Sng"]], meqTechGrp2Sng)
    } else {
      meqTechGrp2Sng <- NULL
    }
    if (!is.null(mTechOutGroup) && !is.null(techSingInp)) {
      meqTechSng2Grp <- merge0(mTechOutGroup, techSingInp)
      obj@parameters[["meqTechSng2Grp"]] <- .dat2par(obj@parameters[["meqTechSng2Grp"]], meqTechSng2Grp)
    } else {
      meqTechSng2Grp <- NULL
    }
    ...
    if (!is.null(techSingInp) && !is.null(techSingOut)) {
      ...
      obj@parameters[["meqTechSng2Sng"]] <- .dat2par(obj@parameters[["meqTechSng2Sng"]], meqTechSng2Sng)
    } else {
      meqTechSng2Sng <- NULL
    }
    ...
    if (!is.null(pTechAf) && any(pTechAf$value != 0 & pTechAf$type == "lo")) {
      obj@parameters[["meqTechAfLo"]] <- .dat2par(
    ...
    obj@parameters[["meqTechAfUp"]] <- .dat2par(obj@parameters[["meqTechAfUp"]], outer_inf(mvTechAct, pTechAf))
    if (!is.null(pTechAfs)) {
      obj@parameters[["meqTechAfsLo"]] <- .dat2par(
        obj@parameters[["meqTechAfsLo"]],
        merge0(mTechSpan, pTechAfs[
          pTechAfs$value != 0 & pTechAfs$type == "lo",
          colnames(pTechAfs)[colnames(pTechAfs) %in% obj@parameters[["meqTechAfsLo"]]@dimSets]
        ])
      )
      meqTechAfsUp <- merge0(
        mTechSpan,
        pTechAfs[pTechAfs$value != Inf & pTechAfs$type == "up", colnames(pTechAfs) %in% obj@parameters[["meqTechAfsUp"]]@dimSets, drop = FALSE]
      )
      obj@parameters[["meqTechAfsUp"]] <- .dat2par(obj@parameters[["meqTechAfsUp"]], meqTechAfsUp)
    }
    if (!is.null(techSingOut)) {
      obj@parameters[["meqTechActSng"]] <- .dat2par(obj@parameters[["meqTechActSng"]], techSingOut)
    } else {
      meqTechActSng <- NULL
    }
    if (!is.null(mTechOutGroup)) {
      obj@parameters[["meqTechActGrp"]] <- .dat2par(obj@parameters[["meqTechActGrp"]], merge0(mvTechAct, mTechOutGroup))
    } else {
      meqTechActGrp <- NULL
    }
    if (!is.null(pTechAfc)) {
      merge_afc <- function(prm, mvTechOut, pTechAfc, type) {
        ...
        if (type == "up") {
          pTechAfc <- pTechAfc[pTechAfc$value != Inf & pTechAfc$type == "up", colnames(pTechAfc) %in% obj@parameters[["meqTechAfcOutLo"]]@dimSets, drop = FALSE]
        } else {
          pTechAfc <- pTechAfc[pTechAfc$value != 0 & pTechAfc$type == "lo", colnames(pTechAfc) %in% obj@parameters[["meqTechAfcOutLo"]]@dimSets, drop = FALSE]
        }
        if (nrow(pTechAfc) == 0) {
          return(prm)
        }
    ...
    if (nrow(tech@weather) > 0) {
      ...
      obj@parameters[["pTechWeatherAfc"]] <- .dat2par(obj@parameters[["pTechWeatherAfc"]], tmp$par)
      obj@parameters[["mTechWeatherAfcUp"]] <- .dat2par(obj@parameters[["mTechWeatherAfcUp"]], tmp$mapup)
      obj@parameters[["mTechWeatherAfcLo"]] <- .dat2par(obj@parameters[["mTechWeatherAfcLo"]], tmp$maplo)
    }
    ...
    if (!is.null(mTechOMCost)) {
      ...
      obj@parameters[["mTechOMCost"]] <- .dat2par(obj@parameters[["mTechOMCost"]], mTechOMCost)
    }
  ...
  function(obj, app, approxim) {
    ...
    if (trd@capacityVariable) {
      ...
      if (trd@olife == Inf) {
        ...
        obj@parameters[["mTradeOlifeInf"]] <- .dat2par(obj@parameters[["mTradeOlifeInf"]], data.frame(trade = trd@name))
      } else {
...
.start_end_fix <- function(approxim, app, als, stock_exist) {
  ...
  for (rr in dend$region) {
    if (any(dd$year > dend[rr, "year"]))
      dd[dd$region == rr & dd$year > dend[rr, "year"], "enable"] <- FALSE
  }
  ...
  for (rr in dlife$region[dlife$region %in% dend$region]) {
    if (any(dd_able$year >= dend[rr, "year"] + dlife[rr, "year"])) {
      dd_able[dd_able$region == rr & dd_able$year >= dend[rr, "year"] +
                dlife[rr, "year"], "enable"] <- FALSE
    }
  }
  dd_eac <- dd_able
  if (nrow(stock_exist) != 0 && any(!dd_able$enable)) {
    for (rr in unique(stock_exist$region)) {
      dd_able[dd_able$region == rr &
                dd_able$year %in% stock_exist[stock_exist$region == rr, "year"],
              "enable"] <- TRUE
    }
  }
  #
...
merge0 <- function(x, y,
  ...
  if (length(by) != 0) {
    ...
    return(as.data.frame(xy)) # debug pDiscountFactorMileStone
  }
  # browser()
  y <- as.data.frame(y) %>% .force_year_class_df()
  x <- as.data.frame(x) %>% .force_year_class_df()
  xy <- merge(x, y)
...
.force_value_class_df <- function(dtf) {
  return(dtf)
  # temporary solution to avoid merging conflicts
  if (!is.null(dtf[["value"]]) && !inherits(dtf, "numeric")) {
...
.subtax_approxim <- function(obj, app, tax, whr, approxim) {
  ...
  approxim$slice <-
    approxim$slice@slice_map[[approxim$commodity_slice_map[[app@comm]]]]
  if (whr == "tax") {
