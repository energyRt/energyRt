...
simpleInterpolation <- function(frm, parameter, mtp, approxim,
  ...
  if (is.null(add_set_name)) {
    ...
    stnd <- mtp@dimSetNames[-(1:length(add_set_name))]
    ...
    for(i in 1:length(add_set_value))
      dd[[add_set_name[i]]] <- add_set_value[i]
    dd <- dd[, c(add_set_name, stnd, 'value'), with = FALSE]
  }
...
multiInterpolation <- function(frm, parameter, mtp, approxim,
  ...
  if (is.null(add_set_name)) {
    ...
    for(i in 1:length(add_set_value))
      dd[[add_set_name[i]]] <- add_set_value[i]
    dd <- dd[, c(add_set_name, stnd, 'type', 'value'), with = FALSE]
  }
