...
.slice_check_data <- function(dtf) {
  if (ncol(dtf) < 2) stop(".setTimeSlices: time-slices data.frame should have more than two columns")
  if (colnames(dtf)[ncol(dtf)] != "share") stop(".setTimeSlices: the name of the last column of time-slices data.frame should be 'share'")
  ...
  if (length(unique(dtf[, 1])) != 1) {
    stop(".setTimeSlices: first slice should be 'ANNUAL' with only one slice")
  }
  ...
  # Check
  if (round(sum(dtf$share), 5) != 1) stop(".setTimeSlices: Sum of slice shares should be equal to one, check: ", sum(dtf$share))
  ll <- apply(dtf[, -ncol(dtf), drop = FALSE], 1, paste, collapse = ".")
  if (anyDuplicated(ll)) {
    stop(paste('.setTimeSlices: duplicates sets of time-slices. ("', paste(ll[duplicated(ll)], collapse = '", "'), '").', sep = ""))
  }
  if (length(ll) != prod(sapply(dtf[, -ncol(dtf), drop = FALSE], function(x) length(unique(x))))) {
    ...
    stop(paste('.setTimeSlices: (empty?) time-slices. ("', paste(dtf2[!(dtf2 %in% ll)], collapse = '", "'), '").', sep = ""))
  }
}
...
.setTimeSlices <- function(slice = NULL, ...) {
  ...
  check_slice <- function(nm) {
    if (any(grep("^[A-z]", nm, invert = TRUE)) || any(gsub("[[:alnum:]]*", "", nm) != "") || anyDuplicated(nm)) {
      ...
      ms2 <- NULL
      if (length(n1) != 0) {
        ms1 <- paste('Check slice names "', paste(n1, collapse = '", "'), '". ', sep = "")
      }
      n2 <- unique(nm[duplicated(nm)])
      if (length(n2) != 0) {
        ms2 <- paste('Check slice names "', paste(n2, collapse = '", "'), '"', sep = "")
      }
  ...
  slice_def <- function(dtf, arg) {
    ...
    if (is.null(names(arg)) || any(names(arg) == "")) {
      stop(paste(".setTimeSlices: Unnamed arguments: ", paste(capture.output(print(arg)), collapse = "\n"), sep = "\n"))
    }
