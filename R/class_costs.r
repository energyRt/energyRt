...
newCosts <- function(name, variable, description = '', mult = NULL, subset = NULL) {
  ...
  if (!is.null(subset)) {
    ...
    subset <- subset[, !apply(is.na(subset), 2, all), with = FALSE]
    obj@subset <- subset
  }
 ...
 # Add mult
  if (!is.null(mult)) {
    ...
    if (is.numeric(mult)) {
      ...
      mult <- mult[, !apply(is.na(mult), 2, all), with = FALSE]
      if (anyDuplicated(mult[, colnames(mult) != 'value', with = FALSE])) 
      ...
      if (any(colnames(mult) %in% colnames(subset))) {
        for (ss in colnames(mult)[colnames(mult) %in% colnames(subset)]) {
          mult <- mult[is.na(mult[[ss]]) | mult[[ss]] %in% 
                         unique(subset[[ss]][!is.na(subset[[ss]])]),]
        }
      }
    ...
    if (any(!fl)) {
      subset_na <- stm@subset[!fl,]
        for (i in seq_len(ncol(subset_na))[apply(is.na(subset_na), 2, any)]) {
          ...
          subset_na <- unique(rbind(subset_na[!fl,], subset_na3))
        }
      subset <- rbind(subset, subset_na)
    }
    for (ss in colnames(subset))
      subset <- subset[subset[[ss]] %in% approxim2[[ss]],]
