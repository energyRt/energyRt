...
newCost <- function(name, variable, description = '', mult = NULL, for.each = NULL, for.sum = NULL) {
  ...
  obj@misc$sets_for.sum <- sets_for.sum
...
.getCostEquation <- function(prec, stm, approxim) {
 ...
 #  
 #  
 #  # Create mult parameter if need
 #  if (nrow(stm@mult) == 0) {
 #      approxim2 <- approxim[unique(c(colnames(stm@mult)[colnames(stm@mult) %in% names(approxim)], 'solver', 'year'))]
 #      if (any(names(approxim2) == 'slice')) {
 #        approxim2$slice <- approxim2$slice@all_slice
 #      }
 #      need.set <- lhs.set2[lhs.set2$set %in% colnames(stm@mult), 'set']
 #      need.set2 <- lhs.set2[!is.na(lhs.set2$new.map) & lhs.set2$set %in% colnames(stm@mult), ]
 #      xx <- newParameter(paste0('pCostMult', stm@name), need.set, 'simple', defVal = stm@defVal, 
