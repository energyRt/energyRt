...
.check_scen_par <- function(scen) {
    ...
    #		scen@modInp@parameters$pTechShare@nValues <- nrow(scen@modInp@parameters$pTechShare@data)
		tmp <- .get_data_slot(scen@modInp@parameters$pTechShare)
		mTechSpan <- .get_data_slot(scen@modInp@parameters$mTechSpan)
		tmp <- .merge_with_null(tmp, mTechSpan)
		tmp_lo <- .merge_with_null(tmp[tmp$type == 'lo',], mTechGroupComm)
		tmp_up <- .merge_with_null(tmp[tmp$type == 'up',], mTechGroupComm)
		tmp_lo <- aggregate(tmp_lo[, 'value'], tmp_lo[, !(colnames(tmp_lo) %in% c('type', 'comm', 'value')), with = FALSE], sum)
		tmp_up <- aggregate(tmp_up[, 'value'], tmp_up[, !(colnames(tmp_up) %in% c('type', 'comm', 'value')), with = FALSE], sum)
