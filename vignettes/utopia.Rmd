...
```{r BASE, eval=T, echo=F}
mdl@sysInfo@debug$comm <- 'ELC'  
save('mdl', file = 'c:/tmp/rr.RData')
mdl@data[[1]]@data$EHYD
...
scen_BASE_glpk <- solve(mdl, name = "BASE_glpk", 
                   # tmp.path = "C:/solwork",
                   # solver = mysolver,
                   solver = "GLPK",
...
scen_BASE_gams <- solve(add(mdl), name = "BASE_gams", 
                   # tmp.path = "C:/solwork",
                   # solver = "GAMS",
                   solver = list(
                     lang = "GAMS",
...
scen_BASE_py <- solve(add(mdl), name = "BASE_py", 
                   # tmp.path = "C:/solwork",
                   # solver = mysolver,
                   solver = list(
                     lang = "Pyomo",
...
scen_BASE_jl <- solve(add(mdl), name = "BASE_jl", 
                   # tmp.path = "C:/solwork",
                   # solver = mysolver,
                   # solver = "JuMP",
                   solver = list(
...
```{r, include=F, eval=F, echo=F}
scen_BASE_int <- interpolate(add(mdl), #, TRBD_ELC_ALL, WWIN, WSOL, EWIN, ESOL, STGELC), 
                         name = "BASE")
summary(scen_BASE_int)
## Writing the model code and data on disk
write_model(scen_BASE_int, solver = "GAMS", tmp.dir = "solwork/UTOPIA_BASE_GAMS")
write_model(scen_BASE_int, solver = "PYOMO", tmp.dir = "solwork/UTOPIA_BASE_PYOMO")
write_model(scen_BASE_int, solver = "GLPK", tmp.dir = "solwork/UTOPIA_BASE_GLPK")
write_model(scen_BASE_int, solver = "JuMP", tmp.dir = "solwork/UTOPIA_BASE_Julia2")

write_model(scen_BASE_int, solver = list(
  lang = "PYOMO", 
  files = list(
    cplex.opt = "blablabla"
  )), 
  tmp.dir = "solwork/UTOPIA_BASE_PYOMO2")


## Solving the model
solve_model(tmp.dir = "solwork/UTOPIA_BASE_GAMS", 
            solver = "GAMS", wait = FALSE, invisible = F)
solve_model(tmp.dir = "solwork/UTOPIA_BASE_PYOMO", 
            solver = "PYOMO", wait = FALSE, invisible = F)
solve_model(tmp.dir = "solwork/UTOPIA_BASE_GLPK", 
            solver = "GLPK", wait = FALSE, invisible = FALSE)
solve_model(tmp.dir = "solwork/UTOPIA_BASE_Julia2", 
            solver = "JULIA", wait = FALSE, invisible = FALSE)

## Read solution
scen_BASE_gms2 <- read_solution(scen_BASE_int, dir.result = "solwork/UTOPIA_BASE_GAMS")
summary(scen_BASE_gms2)
scen_BASE_py2 <- read_solution(scen_BASE_int, dir.result = "solwork/UTOPIA_BASE_PYOMO")
summary(scen_BASE_py2)
scen_BASE_glpk2 <- read_solution(scen_BASE_int, dir.result = "solwork/UTOPIA_BASE_GLPK")
summary(scen_BASE_glpk2)
scen_BASE_jl2 <- read_solution(scen_BASE_int, dir.result = "solwork/UTOPIA_BASE_Julia")
summary(scen_BASE_jl2)
...
## Solving the model
solve_model(tmp.dir = "solwork/UTOPIA_REN", solver = mysolver, wait = TRUE)
