...
# The loop creates trade-objects for every region with neighbors, 
  ...
  trd <- newTrade(trd_nm, 
                  ...
                  trade = data.frame(
                    src = src_nm,
                    dst = dst_nms,
                    ava.up = convert("GWh", "PJ", 10), # Maximum capacity per route in GW
...
scen_GRID <- solve(add(mdl, TRD_ELC_ALL), # Adding trade routes
                   name = "GRID", 
                  # tmp.path = "C:/solwork",
                   solver = mysolver,
...
scen_RBAU <- solve(add(mdl, TRD_ELC_ALL, WWIN, WSOL, EWIN, ESOL, STGELC), 
                   name = "RBAU", 
...
### Trade
```{r, eval=FALSE}
trd <- getData(scen, name = "vTradeIr", comm = "ELC", merge = T)
head(trd)
# There are two sets with names of regions: `src` and `dst`
# to plot the flows we will need to ...
