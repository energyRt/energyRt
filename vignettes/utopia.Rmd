...
if (T) {
  ...
  if (!any(mypks == "gpclib")) install.packages("gpclib", type="source")
...
```{r BASE, eval=T}
mdl@data[[1]]@data$ECOA
scen_BASE <- interpolate(mdl, name = "BASE")

pTechStock <- scen_BASE@modInp@parameters$pTechStock@data
pTechStock[pTechStock$tech == 'ECOA' & pTechStock$region == 'R1', ]
