...
.handle.sum.pyomo.jump <- function(tmp) {
  ...
  paste0(
    "(", .eqt.to.pyomo.jump(substr(a2, 2, nchar(a2))), " for ", .get_pyomo.jump_loop_fast2(substr(a1, 1, nchar(a1) - nchar(a2))), ")",
  ...
  while (nchar(tmp) != 0) {
    ...
    if (substr(tmp, 1, 4) == "sum(") {
      ...
      if (nchar(a2$end) > 1 && substr(a2$end, 1, 1) == "$") {
        ...
        rs <- paste0(
          rs, "(", vrb, "[", arg, "] ", "if ", arg2, " in model.", gsub("([$]|[(].*)", "", a2$end),
...
.equation.from.gams.to.pyomo.AbstractModel <- function(eqt) {
  ...
  if (nchar(declaration) != nchar(gsub("[($].*", "", declaration))) {
    rs <- paste0(
      rs, "model.", gsub("[(].*", "", gsub(".*[$]", "", declaration)), ", rule = lambda model, ",
